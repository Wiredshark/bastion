...
impl ParticleMgr {
            ...
            .join()
        {
            match body {
                ...
                Body::Object(object::Body::NapalmShot) => {
                    self.maintain_napalmshot_particles(scene_data, interpolated.pos, vel)
                },
    ...
    fn maintain_napalmshot_particles(
        ...
        prof_span!("ParticleMgr::maintain_napalmshot_particles");
        let time = scene_data.state.get_time();
        let dt = scene_data.state.get_delta_time();
        let mut rng = rand::rng();
    ...
    fn maintain_napalmpool_particles(&mut self, scene_data: &SceneData, pos: Vec3<f32>) {
        prof_span!("ParticleMgr::maintain_napalmpool_particles");
        let time = scene_data.state.get_time();
        let mut rng = rand::rng();
    ...
    fn maintain_portal_particles(&mut self, scene_data: &SceneData, pos: Vec3<f32>) {
        ...
        for _ in 0..self.scheduler.heartbeats(Duration::from_millis(150)) {
            let outer_pos = pos
                + (Vec2::unit_x().rotated_z(rng.random_range((0.)..PI * 2.))
                                ...
                                self.particles.resize_with(
                                    ...
                                    || {
                                        ...
                                        let outer_pos = interpolated.pos
                                            ...
                                            .normalized()
                                                * (body.max_radius() + 2.0)
                                            + Vec3::unit_z() * body.height() * rng.random::<f32>();
                    ...
                    }) = physics.in_fluid
                    {
                        ...
                        self.particles.resize_with(new_count, || {
                            let start_pos = interpolated.pos
                                ...
                                .map(|d| d * rng.random_range(-10.0..10.0));
                                    ...
                                    || {
                                        let start_pos = interpolated.pos
                                            + (Vec2::unit_y()
                                                * rng.random::<f32>()
                                                * body.max_radius())
                                            .rotated_z(rng.random_range(0.0..(PI * 2.0)))
                                            .with_z(body.height() * rng.random::<f32>());
    ...
    fn maintain_beam_particles(&mut self, scene_data: &SceneData, lights: &mut Vec<Light>) {
        ...
        for (beam, ori) in (&ecs.read_storage::<Beam>(), &ecs.read_storage::<Ori>()).join() {
            ...
            let beam_tick_count = if rng.random_bool(f64::from(beam_tick_count.fract())) {
            ...
            match beam.specifier {
                beam::FrontendSpecifier::Flamethrower => {
                    ...
                    if scene_data.flashing_lights_enabled {
                        lights.push(Light::new(
                            beam.bezier.start,
                            Rgb::new(1.0, 0.25, 0.05).map(|e| e * rng.random_range(0.8..1.2)),
                            2.0,
                        ));
                    }

                    for i in 0..beam_tick_count {
                        let phi: f32 = rng.random_range(0.0..angle);
                ...
                beam::FrontendSpecifier::Cultist => {
                    ...
                    if scene_data.flashing_lights_enabled {
                        lights.push(Light::new(
                            beam.bezier.start,
                            Rgb::new(1.0, 0.0, 1.0).map(|e| e * rng.random_range(0.5..1.0)),
                            2.0,
                        ));
                    }
                    for i in 0..beam_tick_count {
                            ...
                            self.particles.resize_with(
                                ...
                                || {
                                    ...
                                    let z_start = body_maybe
                                        .map_or(0.0, |b| rng.random_range(0.5..0.75) * b.height());
                                    let z_end = body_maybe
                                        .map_or(0.0, |b| rng.random_range(0.0..3.0) * b.height());
                                    Particle::new_directed(
            ...
            if let Ok(chosen) = chosen {
                self.particles.extend(chosen.map(|smoker| {
                    Particle::new(
                        ...
                        time,
                        if rng.random::<f32>() > smoker.dry_chance {
    ...
    fn maintain_pool_particles(&mut self, scene_data: &SceneData) {
        ...
        for (interp, pos, pool) in (
            ecs.read_storage::<Interpolated>().maybe(),
            &ecs.read_storage::<Pos>(),
            &ecs.read_storage::<comp::Pool>(),
        )
            .join()
        {
            let pos = interp.map_or(pos.0, |i| i.pos);
