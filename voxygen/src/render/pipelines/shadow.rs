...
pub struct ShadowFigureBatchPipeline {
    pub pipeline: wgpu::RenderPipeline,
}

impl ShadowFigureBatchPipeline {
    pub fn new(
        device: &wgpu::Device,
        ...
        let layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("Bastion directed figure batch shadow layout"),
            push_constant_ranges: &[],
            bind_group_layouts: &[&global_layout.globals, &figure_layout.batch_instances],
        });
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("Bastion directed figure batch shadow pipeline"),
            layout: Some(&layout),
