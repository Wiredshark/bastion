...
/// A deterministic, human-readable rendering of a real, generated
/// certificate — pure function of its fields (already canonically
/// sorted by [`generate_certificate_v1`], never re-sorted or otherwise
/// re-derived here), so two runs against identical evidence produce
/// byte-identical output. This is what "the certificate needs to be RUN,
/// not merely runnable" produces: a real artifact this program can
/// commit and diff, not a summary written by hand.
pub fn render_certificate_v1(cert: &ApexCertificateV1) -> String {
    let mut out = String::new();
    out.push_str("# APEX Certificate v1\n\n");
    out.push_str("Generated by `common::apex::certificate::generate_certificate_v1` from real, cited attestations.\n\n");

    out.push_str("## Roots\n\n");
    out.push_str(&format!("Present: {} / {}\n\n", cert.present_roots.len(), ApexCertificateRootIdV1::ALL.len()));
    for (root, digest, source) in &cert.present_roots {
        out.push_str(&format!("- PRESENT `{root:?}` = `{}` — {source}\n", digest.bytes.to_human_v1()));
    }
    for (root, reason) in &cert.absent_roots {
        out.push_str(&format!("- ABSENT `{root:?}` ({}): {}\n", reason.id, reason.reason));
    }

    out.push_str("\n## Certified properties\n\n");
    out.push_str(&format!("Stated: {} / {}\n\n", cert.certified_properties.len(), CertifiedPropertyIdV1::ALL.len()));
    for property in &cert.certified_properties {
        out.push_str(&format!(
            "- `{:?}`: {}/{} covered — {}\n",
            property.property,
            property.covered_cases,
            property.total_cases,
            property.sources.join(", ")
        ));
    }

    let absent_properties: Vec<CertifiedPropertyIdV1> =
        CertifiedPropertyIdV1::ALL.into_iter().filter(|p| !cert.certified_properties.iter().any(|c| c.property == *p)).collect();
    out.push_str(&format!("\nStructurally absent (zero covered cases): {}\n", absent_properties.len()));
    for property in &absent_properties {
        out.push_str(&format!("- `{property:?}`\n"));
    }

    out.push_str(&format!("\n## Open set ({} cases)\n\n", cert.open_set.len()));
    for (property, case) in &cert.open_set {
        out.push_str(&format!("- `{property:?}` / `{}`: {}\n", case.id, case.reason));
    }
...
mod tests {
    ...
    #[test]
    fn rendering_the_same_certificate_twice_is_byte_identical() {
        let roots = vec![present_root(ApexCertificateRootIdV1::Content, 1), absent_root(ApexCertificateRootIdV1::Output)];
        let attestations = vec![fully_covered(CertifiedPropertyIdV1::MultiStoreCrashCutpoints, 5)];
        let cert = generate_certificate_v1(&roots, &attestations);
        assert_eq!(render_certificate_v1(&cert), render_certificate_v1(&cert));
    }

    /// The rendering names every present root's digest, every absent
    /// root's reason, every stated property, and every open case --
    /// nothing silently dropped between the generated struct and its
    /// rendered text.
    #[test]
    fn the_rendering_names_every_root_property_and_open_case() {
        let roots = vec![present_root(ApexCertificateRootIdV1::Content, 7), absent_root(ApexCertificateRootIdV1::Output)];
        let attestations = vec![PropertyAttestationV1::new(
            CertifiedPropertyIdV1::CrossTargetExecution,
            1,
            0,
            vec![open_case("CROSS-TARGET-EXECUTION", "no dual-cell execution exists")],
            vec!["T6.4", "T8.2"],
        )
        .unwrap()];
        let cert = generate_certificate_v1(&roots, &attestations);
        let rendered = render_certificate_v1(&cert);

        assert!(rendered.contains(&digest(7).bytes.to_human_v1()), "present root's digest must appear verbatim");
        assert!(rendered.contains("Output"), "absent root's id must appear");
        assert!(rendered.contains("TEST-ABSENT"), "absent root's reason id must appear");
        assert!(rendered.contains("CrossTargetExecution"), "structurally-absent property must be named");
        assert!(rendered.contains("CROSS-TARGET-EXECUTION"), "every open case id must appear");
        assert!(rendered.contains("no dual-cell execution exists"), "every open case reason must appear");
    }
}
