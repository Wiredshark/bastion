...
const CATALOG: &[SelectionSiteV1] = &[
    ...
    site("server/src/lib.rs", 0, ".min_by_key(|(_, j)| (j.pos.z - cell.z).abs())", IncompleteCosmetic, "bastion_inspect_cell, explicitly documented READ-ONLY (a right-click inspect tooltip) -- a tie only changes which equidistant job's info is shown, not any simulated state. Literal corrected 2026-08-09 (Class-1 catalog batch): registry had the closure without its tuple-destructure prefix; live code has always taken a (key, value) pair here per the preceding .filter(|(_, j)| ...) in the same chain -- not re-verified why the earlier scan recorded it short, only that the site's own semantics are unchanged"),
    ...
    site("server/src/sys/msg/in_game.rs", 0, ".min_by_key(|(_, j)| (j.pos.z - cell.z).abs())", IncompleteCosmetic, "same bastion_inspect_cell selection as the server/src/lib.rs site above (mirrored implementation, both explicitly READ-ONLY inspect tooltips) -- literal corrected 2026-08-09, same reason as that entry"),
    ...
    // E14-5 chunk 3: bastion-harness/src entered the root set -- THE
    // INSTRUMENT, taken last and only after its three real defects were
    // fixed (E14-5a). All three sites were already read during chunk
    // 2's per-site pass; the verdicts are carried in rather than
    // re-derived.
    site("bastion-harness/src/asset_test.rs", 0, ".min_by_key(|(b, _)| **b)", Complete, "work-marker cell pick over authored_markers, keyed by the marker byte itself -- the key IS the map's key, so it is unique by construction and NO TIE IS POSSIBLE. Same argument as common/state's dependency-cycle rotation"),
    site("bastion-harness/src/main.rs", 0, ".max_by_key(|p| {", CompleteByOrderedInput, "site-centre pick by neighbour count over `grounded`, a Vec collected from a filter. No tiebreak in the key, so equal-neighbour candidates resolve by max_by_key's take-the-LAST rule over a deterministic Vec order"),
    site("bastion-harness/src/main.rs", 0, ".min_by(|&a, &b| {", CompleteByOrderedInput, "floor-subject pick over `holders: Vec<usize>` comparing expected[] via partial_cmp. Input is an ordered Vec; a NaN would compare Equal through unwrap_or and likewise land on the first. Harness scenario selection, not simulation state"),
