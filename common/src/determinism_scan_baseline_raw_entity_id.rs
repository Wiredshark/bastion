[
    ...
    ("common/systems/src/phys/mod.rs", "// Uid (not the raw ECS `entity.id()` allocator slot --", 0),
    ("common/systems/src/phys/mod.rs", "// in `land_on_ground` at SORT time, the other off `entity.id()`", 0),
    ("common/systems/src/phys/mod.rs", "// this one by re-reading `entity.id()` off the raw `Entity` stored", 0),
