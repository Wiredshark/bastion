...
/// signal, not a plain timer). Own constant, not a reuse of
/// `PERSIST_ESCALATE_STRIKES` under a borrowed name -- same discipline
/// as the strike/grant distinction the withdrawn Row B already
/// established, now strike-count vs tick-duration.
///
/// 120 = a number carried over from the withdrawn Row B's 3-grant debt
/// graduating across ticks 2940→3060 on a seed-90 run -- named
/// precisely so the provenance isn't overstated: that observation was
/// taken ON THE SAME BUILD whose per-tick grant-loop we have since
/// PROVEN perturbed timing enough to manufacture a threshold crossing
/// on seed 76 (the reason Row B was withdrawn). The number is real, but
/// it was measured on a system running detectably differently from the
/// one B′ runs on -- not a clean transplant, a borrowed starting point.
/// UNVALIDATED DEFAULT for THIS mechanism on every count: Row B's
/// corpus result judged its per-tick COST, never this duration: the
/// two designs graduate on different clocks (grants vs ticks) with no
/// guaranteed correspondence; and the clock itself was perturbed when
/// this number was taken off it. The paired A/B is what judges it, same
/// discipline as `PERSIST_ESCALATE_STRIKES` and the withdrawn
/// `BENCH_AMNESTY_GRANTS_OWED` before it -- sufficiency claims name
/// their case, and this one's case is "carried over from a system we
/// now know was running slightly differently," not "measured on B′."
pub const ROWB_BENCH_TICKS: u64 = 120;
...
fn has_standable_stance(terrain: &TerrainGrid, pos: Vec3<i32>) -> Option<Vec3<i32>> {
    ...
    // 2. ADJACENT-GROUND fallback — see `adjacent_ground_stance`.
    adjacent_ground_stance(terrain, pos)
}

/// bastion (task #64, extracted from `has_standable_stance`'s step 2): a
/// cardinal neighbor cell at `pos`'s own level with a solid floor below +
/// open feet + open head — stand there and work sideways. This is the
/// reachable stance a wedged `+1`-gap block has downhill, the way a
/// reachable floating LEDGE is worked, AND — new for task #64 — the
/// PRIMARY (not fallback) stance for `AdjacentToBase`: `pos` itself is
/// empty for that affordance class (Build/Bed/a Ladder base), so on-top
/// is never tried there at all; this tier is the whole answer. An
/// ISOLATED floater has none (its neighbors' floors are air) → `None` →
/// clean-SKIP (no claim→unreachable churn; deferred to cave-in for
/// SolidTarget, left unclaimed-and-retried for AdjacentToBase).
fn adjacent_ground_stance(terrain: &TerrainGrid, pos: Vec3<i32>) -> Option<Vec3<i32>> {
    let solid = |p: Vec3<i32>| terrain.get(p).map(|b| b.is_filled()).unwrap_or(false);
    let open = |p: Vec3<i32>| terrain.get(p).map(|b| !b.is_filled()).unwrap_or(false);
    let cardinals = [
        Vec2::new(1, 0),
...
/// thing to reach onto or beside. Feet land AT `pos` (offset zero);
/// support is the solid cell directly below. Re-verified here defensively
/// rather than trusted from the farm pass's own precondition — a stance
/// function's whole job is answering "can a colonist really stand here",
/// not assuming its caller already checked.
fn at_target_stance(terrain: &TerrainGrid, pos: Vec3<i32>) -> Option<Vec3<i32>> {
    let solid = |p: Vec3<i32>| terrain.get(p).map(|b| b.is_filled()).unwrap_or(false);
    let open = |p: Vec3<i32>| terrain.get(p).map(|b| !b.is_filled()).unwrap_or(false);
    (open(pos) && open(pos + Vec3::unit_z()) && solid(pos - Vec3::unit_z())).then_some(Vec3::zero())
}

/// bastion (task #64, KindAffordance): the SINGLE dispatcher every stance
/// site now calls — replaces the old Chop/Mine-only special-casing in the
/// stance loop, the blind `unwrap_or(Vec3::unit_z())` default, and the
/// Mine-only leave-unclaimed guard all at once (see the packet: sites #1
/// stance loop, #2 on-top default, #3 leave-unclaimed guard, all
    ...
    fn run(
                        ...
                        if sdist + STUCK_EPSILON < active.best_dist {
                            ...
                            if active.stuck_time > STUCK_TIMEOUT {
                                ...
                                *board.timeout_counts_by_pos.entry(job.pos).or_insert(0) += 1;
                                ...
                                // ROW B′ (2026-08-04, replaces the
                                // withdrawn Row B -- see Job::
                                // benched_until_tick's own doc for the
                                // full mechanism and why it's a tick, not
                                // a grant-count map). The SAME gate as
                                // route_exhausted above, deliberately --
                                // both fire off one condition. Only sets
                                // the field when it's currently `None`: a
                                // job already benched (still above
                                // threshold on a later hit, which can
                                // only happen after graduating and
                                // climbing back past the line) does NOT
                                // get its window extended -- one
                                // comparison, one store, no map, no
                                // per-tick work of any kind at this site
                                // or the amnesty loop that reads it.
                                if rowb_bench_enabled()
                                    && job.stuck_strikes >= PERSIST_ESCALATE_STRIKES
                                    && job.benched_until_tick.is_none()
                                {
                                    job.benched_until_tick = Some(tick.0 + ROWB_BENCH_TICKS);
                                    if std::env::var_os("BASTION_ROWB_DIAG").is_some() {
        ...
        if tick.0 % (ARBITRATION_INTERVAL * 4) == 0 {
            ...
            if grant_amnesty {
                ...
                // resetting `unreachable` -- #60 budget: the bench adds
                // ONE comparison per job already in hand, no new
                // iteration, no map, no per-grant maintenance. A
                // benched job (`benched_until_tick.is_some()`) sits THIS
                // grant out unless its window has elapsed; graduating
                // clears the field in the same pass so a resolved job
                ...
                for (id, job) in board.jobs.iter_mut() {
                    match job.benched_until_tick {
                        Some(until) if tick.0 < until => {
                            // Still benched -- sits this grant out.
                        },
                        Some(_) => {
                            job.benched_until_tick = None;
                            job.unreachable = false;
                            if diag {
                                info!(job = *id, pos = ?job.pos, tick = tick.0, "ROWB-DIAG: graduated");
                            }
                        },
        ...
        // next cycle.
        // TASK #64 (job-kind affordance table, 2026-08-03): generalized from
        // the Chop/Mine-only loop above (which the #56/#62 comment
        // documents) to EVERY kind, dispatching on `job.affordance` instead
        // of hand-matching `job.kind`. Mine keeps its own exposure
        // pre-check below (a MINE-SPECIFIC concept — digging into a solid
        // mass that hasn't opened a face yet; every other affordance
        // class's target is already open air by `job_wanted`'s own
        // definition, or — Chop — open air by construction, so an
        // "enclosed" state doesn't apply the same way and isn't gated
        // separately: `job_stance`'s own physical requirements already
        // return `None` for a genuinely buried/unreachable target).
        for (id, job) in board.jobs.iter_mut() {
            ...
            if job.kind.is(DesignationKind::Mine) {
                ...
                // gated (they use the on-top stance, as before).
                if job.is_access {
                    standable.insert(*id, Vec3::unit_z());
                    continue;
                }
            }
            // Ordinary Mine cells, and now every other kind, must have a
            // real stance; a cell with none (isolated floater / walled `+1`
            // gap / a Ladder rung whose support doesn't exist yet) is left
            // UNCLAIMED this cycle — NOT flagged unreachable, so no
            // claim→unreachable churn — and retried each cycle as the
        ...
        for (entity, uid) in claim_order {
            ...
            for id in board.decision_job_ids() {
                ...
                let sat_penalty = board
                    ...
                    .unwrap_or(0.0)
                    * COORD_SAT_WEIGHT;
                // TASK #59 (aging) REVERTED, 2026-07-30 (Fable's ruling,
                // wave 15): confirmed corpus regression at 48-seed VM
                // scale (14/48 -> 16/48 all-clause; 2 newly-broken seeds,
                // 0 rescued) with zero demonstrated rescue anywhere it was
                // tested. Reverting to the exact pre-aging score formula.
                // The starvation/times_offered instrumentation this fed
                // from (cycles_since_last_claim et al.) is UNTOUCHED --
                // still populated every arbitration cycle, still report-
                // only, just no longer read here. Greedy-scheduling
                // starvation remains a real, demonstrated mechanism
                // (residual cells sitting unclaimed for up to 360 cycles
                // with a crowded field the whole time); the fix attempt
                // was wrong, not the diagnosis. Re-landing aging needs the
            ...
            if let Some((job_id, _, _)) = best {
                ...
                // longer a blind on-top default for anything outside
                // Chop/Mine. The generalized leave-unclaimed guard above
                // already guarantees a real entry exists here for every
                // affordance except `Untargeted` (whose own `job_stance` arm
                // always answers `Some(unit_z())`); `unwrap_or` stays as a
