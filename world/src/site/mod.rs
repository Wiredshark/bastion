...
impl Site {
    ...
    pub fn generate_mine(land: &Land, rng: &mut impl Rng, origin: Vec2<i32>) -> Self {
        ...
        site.name = Some(dwarven_mine.name().to_string());
    ...
    pub fn generate_gnarling(land: &Land, rng: &mut impl Rng, origin: Vec2<i32>) -> Self {
        ...
        site.name = Some(gnarling_fortification.name().to_string());
    ...
    pub fn generate_adlet(
        ...
        site.name = Some(adlet_stronghold.name().to_string());
    ...
    pub fn generate_giant_tree(land: &Land, rng: &mut impl Rng, origin: Vec2<i32>) -> Self {
        ...
        site.name = Some(giant_tree.name().to_string());
    ...
    pub fn generate_chapel_site(land: &Land, rng: &mut impl Rng, origin: Vec2<i32>) -> Self {
        ...
        let mut site = Site {
            origin,
            name: Some(NameGen::location(&mut rng).generate_danari()),
                            ...
                            for z in aabb.min.z..aabb.max.z {
                                ...
                                let map = |block| {
                                    ...
                                    if let Some(spec) = entity_path {
                                        ...
                                        // entropy (`rand::rng()`), so
                                        // authoritative equipment varied
                                        // run-to-run even under deterministic
                                        // worldgen. Key it by construction on
                                        // (world seed, entity WORLD position)
                                        // + a loadout domain salt — the
                                        // position is intrinsic to the
                                        // structure, so this is stable and
                                        // independent of generation/iteration
                                        // order.
