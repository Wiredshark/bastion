use crate::{
    ...
    site::{Fill, Painter, Site, Structure, namegen::NameGen},
    util::FastNoise,
};
use common::{
...
impl GiantTree {
    ...
    pub fn entity_at(
        ...
        if above_block.kind() == BlockKind::Leaves && dynamic_rng.random_bool(0.000055) {
            let entity = EntityInfo::at(pos.as_(), &mut *dynamic_rng);
...
impl Structure for GiantTree {
    #[cfg(feature = "dyn-lib")]
    #[unsafe(export_name = "as_dyn_structure_gianttree")]
    fn as_dyn_outer(&self) -> Option<(&dyn Structure, &'static str)> {
        Some((Self::as_dyn_impl(self), "as_dyn_structure_gianttree"))
    }
