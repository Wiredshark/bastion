...
impl Structure for TrollCave {
    #[cfg(feature = "dyn-lib")]
    #[unsafe(export_name = "as_dyn_structure_trollcave")]
    fn as_dyn_outer(&self) -> Option<(&dyn Structure, &'static str)> {
        Some((Self::as_dyn_impl(self), "as_dyn_structure_trollcave"))
    }
    ...
    fn render_inner(&self, _site: &Site, land: &Land, painter: &Painter) {
        ...
        // troll
        painter.spawn(
            EntityInfo::at(center.with_z(base - 15).as_(), &mut rng).with_asset_expect(troll, &mut rng, None),
        );
        // bat
        painter.spawn(
            EntityInfo::at((center - 2).with_z(base + 5).as_(), &mut rng).with_asset_expect(
