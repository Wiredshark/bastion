...
impl CoastalAirshipDock {
    pub fn generate(
        ...
        let mut alt = min_foundation_alt + 2;
        let size = 20;
        let bldg_height = 12;
        let mut base = alt + 1;
        let mut top_floor = base + (bldg_height * 6) - 3;
...
impl Structure for CoastalAirshipDock {
    #[cfg(feature = "dyn-lib")]
    #[unsafe(export_name = "as_dyn_structure_coastalairshipdock")]
    fn as_dyn_outer(&self) -> Option<(&dyn Structure, &'static str)> {
        Some((
            Self::as_dyn_impl(self),
            "as_dyn_structure_coastalairshipdock",
        ))
    }
