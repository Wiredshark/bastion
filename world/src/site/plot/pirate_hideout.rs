...
impl Structure for PirateHideout {
    #[cfg(feature = "dyn-lib")]
    #[unsafe(export_name = "as_dyn_structure_piratehideout")]
    fn as_dyn_outer(&self) -> Option<(&dyn Structure, &'static str)> {
        Some((Self::as_dyn_impl(self), "as_dyn_structure_piratehideout"))
    }

    fn render_inner(&self, _site: &Site, land: &Land, painter: &Painter) {
        let center = self.bounds.center();
        let base = land.get_alt_approx(center) as i32;
        let mut rng = super::plot_render_rng(self.bounds.center(), 0x91A7_0018);
