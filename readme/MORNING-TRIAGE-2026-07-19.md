...
## ★ BUILD-SPEED TASK (Ben-directed, ~05:00) — "builder sat around waiting, wasting tokens"; fix the WAIT, not the compile
Compile is already tuned (sccache + rust-lld + incremental + `bastion-check` scoped alias — done). The waste is elsewhere; 4 concrete levers, do in order:
1. [HIGH, zero-build-cost] VERIFICATION defaults to BASTION_FLAT_ARENA (already fully wired, main.rs ~8812-8940 — boots in seconds vs the ~74s full-worldgen boot). Audit which harness scenarios boot full worldgen when the flat arena would prove the same thing; flip those to flat-boot. Keep full worldgen ONLY for tests that need real terrain (some ladder/mine fixtures). This is where most "sat around testing" time went.
2. [HIGH, one config block, SCHEDULED — busts warm caches = one full rebuild fleet-wide, so do it deliberately] Add a test profile with `lto = false` (inherit release's opt-level=3 or drop to 2). Harness verification builds `--release` today and pays the full LTO link tax for a binary that only needs to run a sim. LTO stays on for shipped-game builds only.
