...
|---|---|---|---|---|---|---|
| ITEM8-V5 (**ARC 1 CLOSED**) | binary `4d918025`, capture `8dd2f9463b` | `bastion_jobs.rs`: completion arm's 4 `watch_wipe` sites (2 newly gated), `handle_exhausted_replan` wiring (extracted so the WIRING is testable, not just the shared method), `completion_block`; 11-consumer enumeration of the completion signal | **PASS — 7 pass · 1 partial · 0 fail.** F8 held PARTIAL (exclusion half proven 33×, inclusion half unexercised — zero on an unreachable channel is not a pass). Arc-1 close **recommended and accepted** | Gate 0 ×2 read from log; F1 on the **revised** final-third bar (last farm event 0.6 s before last log line); F7 **3.98%** on the UNION of 3 completion channels (v4: 98.6%); flat-reap falsifier passed in its **registered** form (claims 17×, reaps 0×); food terminal-streak **0** vs v4's **517**; rescues 20 uids/max 6 (loop hypothesis refuted); N=8 compression certification (6×, zero overlap) | **defect 1** foliage-persistence → task #86 / `ROW-DEFECT1-FOLIAGE-PERSISTENCE.md` · **F8 inclusion half** (5b, one-Mine-job fixture) · completion-signal split · S1 promotion to scored bar · #73 script the teardown | `ITEM8-V5-RESULTS.md`, `DOSSIER-ARC1-COLONY-SURVIVAL.md` |
...
| APEX-T3.3.13 | `acb2bbdb06` (apex-t0, PRE-MERGE) | `server/src/sys/entity_sync.rs`: `try_enqueue_entity_sync_intent` + 5 intent call sites in the rayon region closure (Create/Delete/paired EntitySync+CompSync/throttle CompSync); `client.rs` `semantic_send_state()` getter | PASS pre-merge (elevated gate; 1 advisory follow-up, non-blocking) — verbatim legacy fallbacks, throttle fns untouched, V1 unreachable-by-construction (T3.3.05), disclosed inert force-counter delta correctly classified | Independently rerun by Opus in `.apex-t0-wt` at tip: `cargo test -p veloren-server --lib semantic_intents` 7/7 (incl. 1/2/8-worker + region-permutation tapes + deterministic falsifier); full lib suite 66/66 | ADVISORY — **closed by 08160c8a86** (shared named-ordinals module across real sites + tests; sequential CompSync sites migrated in the same pass per the .14 fold-in) | (this row; verdict sent to Fable+Sonnet 2026-07-27) |
