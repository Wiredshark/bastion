...
| bastion-block-ENGOPT7 | a4018f948a | Ledger #183 (Chaser no-path negative cache, SHIPPED) + #179 (Small→Medium continuation equivalence, executable negative, no fix), self-gated, no master-list row. Branch `bastion/builder`. Named ENGOPT7 deliberately out of strict landing order to preserve the still-open residual's in-tree ENGOPT6 name (Builder 4 flagged the choice explicitly). #183: a completed empty-frontier verdict against an unchanged (target, #178-profile) question suppresses re-search; invalidation on target-move/profile-flip/InvalidPath/arrival/90-tick half-open re-probe (circuit-breaker prior art, not a permanent cache — #184 revision plumbing not in yet). Falsifier fired unfixed (200 searches/200 ticks), pinned ≤5 fixed. PATH-0/movement-fallback contract preserved. veloren-common 155/156 (B59 pre-existing only), bastion-server 36/36. #179: sharp precondition PROVEN engaged via a test-only Astar::visited() accessor (not vacuous like #181/B65), still came back negative — ENGOPT2's strict-improvement re-push heals the mixed frontier; test kept as a permanent equivalence pin, no code shipped | revert tag bastion-block-ENGOPT4 (7b994ea99c) — NOTE: the ENGOPT6 residual (fix `3b137017e6`) sits between ENGOPT4 and this tag chronologically but is UNTAGGED pending its end-proof fan; once confirmed it inserts here in the revert chain, not after | `Chaser` gains negative-cache state (server-internal search-scheduler bookkeeping) — no save/wire-shape change; #179 ships zero production code, test-only |
