...
### TR1d's first day on both arms (11:02, b75fc10a which carries TR1d): every field with no road within reach ran into the +-24 box -- b2: 4 "boxed" (704-1,829 cells, road None) and 7 "open" (all boxed=true too, 926-2,521 cells); b1: 11 "open" (all boxed=true, 1,016-2,264 cells); "closed" 0 and 0. THE STORE'S 1,926 CELLS WERE THE BOX, NOT A WALL. The probe's third pair (the store) printed nothing: store_p was None and the code skips a None pair silently

- Rim half (wait-probe-tr1d by6gte9yr is still waiting for `label="store"`; this read is by hand from both live logs at day 1 hour ~5 (b2) and day 0 hour 20 (b1)): 11 rim lines per arm, all carrying `boxed=`. No verdict "closed" anywhere today. Bar: the store reads closed+agree-cut (its door) or boxed+agree-path (the box): the rim half says BOXED; W14-j's "closed 1,926 cells, no road" for the hauler inside the general store (07:55) was the census's own box edge, and the same for b1's 1,596 and 2,092 (09:28). The general store's interior is not walled off from the road for the router's steps.
- Probe half: ROUTER PROBE printed "upstairs" (agree-path, 5 polls) and "ground" (agree-path, 3 polls) on b2 at day 0 and day 1, no "store" line: `store_p` (the first non-private stockpile's floor centre at the first walkable z in min.z..=min.z+3) was None -- the general store's centre column has no walkable cell in that band (W17-c: the store is a field of crates), and `for (label, bed) in [..]` skips a None without a word. NOT EVIDENCED, and an instrument defect: the None needs a witness (fold into the next probe row: THE PROBE NAMES THE PAIR IT COULD NOT ASK; pick the store's nearest walkable aisle cell instead of its centre).
- Disposition: the rim half answers the question TR1d was built for (the box, not the door); the probe half is unevidenced by its own silence and is reported so.

### E2-v's first day and night on b2 (10:55, b75fc10a, day 1 hour 3): THE DELIVERY OF NOTHING 0 (from 5 the day before), haul delivered 121 with amount=0 0, NO CARGO 18, double-claimed 11 of 221 (from 21 of 226), night shelves empty over a gone load 0 -- ALL THREE BARS MET on the first replicate

- wait-cargo2-e2v b7oryokmf, b2 (boarded 10:28). NO CARGO 18 -- the first four are one job (367, cargo Uid(1040)) refused by colonists 13, 41, 13 and 17 within four seconds: a load neither carried nor standing in the world is offered every pass and refused every pass (cheap, but the job never ends; the sweep named 5 unclaimed loads). CARGO KEPT 0. NIGHT in_bed 3/32/41/42/42/42 at hours 22..3, to_bed 31/9/1/1/1/1. b1 (boarded 10:39, running slow under the builds) reads at its day 1 hour 3, ~11:15.
- Disposition (first replicate): bar (1) deliveries of nothing <= 1 per arm-day: MET (0); bar (2) NO CARGO >= 2: MET (18); bar (3) deliveries with amount > 0 >= 50: MET (121). E2-w's sizing: the refused job is re-offered every pass; the load that left with a stuck hauler sits in its pack. Two shapes for E2-w: (a) drop the carried load at the stuck-timeout release so the world holds it again (the claim then admits any hauler); (b) a Haul refused N passes with no cargo anywhere is removed from the board. Decide on b1's replicate and the refusal counts.

### FS1 LANDED (10:56): 9ead52314c, marker 'THE FAIL-SAFE WAITS FOR THE DIGGERS' server=1 in stage-bin

- Chain basyiy3i1 (fired 10:38 on e2v-staged +300 s): E free 30.7 GB, patch applied (crlf=False), check ok, pin the_fail_safe_waits_for_the_diggers ok (1 passed), commit 9ead52314c (10:43), both halves built, staged 10:56:16. Mechanism: `failsafe_holds(cells_pending, stuck_secs, limit_secs)` -- while at least one emergency access cell is pending for the colonist (board.emergency_access_cells, owner-keyed) and its stuck clock is under STUCK_TELEPORT_SECS + RESCUE_PATIENCE_SECS (60 + 300 s), the ultimate fail-safe does not teleport; THE FAIL-SAFE WAITS FOR THE DIGGERS every 30 s of the hold; the teleport line gains rescue_cells_pending; the plan's teardown witnesses THE RESCUE PLAN ENDED with the owner's stuck clock. Falsifier b9e64xk5s fires +90 s (plants a hold that never holds); reader wait-failsafe-fs1 bwjech25c reads hour 3 of day 1 after the arms board it. Bars: teleports with rescue_cells_pending > 0 = 0 per arm-day; WAITS >= 1 when a plan is pending past 60 s (rare: one to three below-grade events per arm-day; NOT evidenced if none occurs); THE RESCUE PLAN ENDED names the dig's duration. NW1's chain b22hkx4xz fires 11:03:16 (+420 s, past this falsifier's detached-HEAD window).

### A1's first day and night (10:33, both arms on b1018162 which carries A1, day 1 hour 3): access anchors inside houses 0 and 0; mine completions inside houses 0 of 177 (b1) and 0 of 150 (b2); Bed cut 0 of 116 on both; the mask was built once on b1 (claims 0, designations 72, spared 72) and never on b2; descent plans 0 (no deep mine job: the quarry's slab is one deep)

- wait-access-a1 by0nxnewz. b2 (10:22): A1 lines 0; ANCHORS 0 (inside houses 0); MINE completions 150, inside houses 0; CENSUS day 1 connected 56 cut 0, Bed (116, 0); PIT stalls 0; WAITS 0; NIGHT in_bed 3/31/39/42/42/43 at hours 22..3, to_bed 29/10/3/2/1/1. b1 (10:32): A1 lines 1 -- claims=0 designations=72 spared=72 pass=1 (the one access-plan call of the day found no Mine or Ladder claim to dig within, so its mask was empty and every designation, houses included, was spared); ANCHORS 0; MINE 177, inside houses 0; CENSUS connected 56 cut 0, Bed (116, 0); NIGHT 5/33/43/41/42/41, to_bed 24/9/0/2/1/2.
- Disposition: bar (1) access anchors inside houses 0: MET on both arms (with Q1 no mining reaches a house, so no rescue is planned near one; A1 is the guard for the day one is). Bar (2) the planner still emits access for deep mine jobs: NOT EVIDENCED -- no occasion (the quarry's slab is one block deep; descent emitted 0). The one mask built on b1 had claims=0: a colonist asked for access when the town held no Mine or Ladder claim, and got none -- the same "self-rescue found no access" b2 printed twice yesterday; that is the class FS1 does not touch (no plan, no hold) and a dug stair would (see the handoff).

### W6-K's first nights (10:30, both arms on b1018162, day 1 hour 2): two walkers per arm at hour 1, all moving (clock 0.03), none standing; on BOTH arms one of the two is bound for the upstairs bed at (7724,6368,186) with route=None and push=chaser-probed, exhausts 0

- wait-walking-w6k beud4mi5p. b2 (10:21): NIGHT hours 21..2 in_bed 1/3/31/39/42/42, to_bed 3/29/10/3/2/1; WALKING at hour 1: 2 -- colonist 25 at 22 blocks from bed (7574,6386,186), speed 4.20, route Some((64,16)) aged 36,150 ticks, push chaser-pure-glide, pending false, exh 0; colonist 63 at 122 blocks from bed (7724,6368,186), speed 0.58, route None, age 36,150, push chaser-probed, pending false, exh 0. STANDING 0; rims 11; UNCLIMBED 0; WAITS 0; PIT stalls 0. b1 (10:30): in_bed 1/5/33/43/41/42, to_bed 5/24/9/0/2/1; WALKING 2 -- colonist 122 at 26 blocks from (7724,6368,186), speed 4.20, route None, age 36,002, push chaser-probed, exh 0; colonist 1360 at 16 blocks from (7644,6477,186), route Some((57,6)), age None, push chaser-pure-glide. STANDING 0.
- Reading: the far class is two bodies per arm-night and they are WALKING, not stuck; half of it is one bed: (7724,6368,186), an upstairs bed (W6-J's b1 read named colonist 73 never arriving at (7724,6369,186) with 16 fill-exhausts; tonight's walkers to it have exh 0 and route None under push chaser-probed). The probe that finds no route strikes nothing (H2-p's strike counts exhausted approaches); and the arms (and Ben's PLAY.ps1 worlds) boot fresh each day, so a three-strike memory never accumulates. Candidate row: THE PROBE THAT FINDS NO ROUTE STRIKES THE BED (a same-night strike from chaser-probed with route None over N ticks), after FS1.

### E2-v LANDED (10:22): b75fc10a3b, marker 'THE HAUL HAS NO CARGO' server=1 in stage-bin

- Chain bo6392uuh (fired 10:01:49 on tr1d-staged +300 s, 13 s after TR1d's falsifier restored): E free 30.7 GB, patch applied (crlf=False), check ok, pin the_haul_has_no_cargo ok (1 passed), commit b75fc10a3b (10:09), both halves built, staged 10:21:49. Mechanism: in the claim candidate filter a Haul job keeps its priority only if `haul_claim_admitted(carried, cargo_in_world)` -- the claimant carries the job's required item, or the job's cargo entity still stands in the world as a pickup; otherwise priority 0 and THE HAUL HAS NO CARGO is witnessed (job, colonist, cargo uid, required item, refused count). Falsifier bwj4u1609 fires +90 s (plants `carried || cargo_in_world || true`); reader wait-cargo2-e2v b7oryokmf reads hour 3 of day 1 after the arms board it. Bars: deliveries of nothing <= 1 per arm-day (from 3-6); NO CARGO >= 2; deliveries with amount > 0 >= 50. The arms' daily cascades restart b1 ~10:25 and b2 ~10:31, onto E2-v (it carries TR1d, W6-K and A1: wait-probe-tr1d, wait-walking-w6k and wait-access-a1 all read this boot).
- Falsifier bwj4u1609 (10:23-10:26): planted `carried || cargo_in_world || true` (every haul claim admitted): pin the_haul_has_no_cargo went RED (good); restored 0 dirty files. Shipper bkbmp8an9 shipped b75fc10a3b to lab-bin at 10:22:29: THE PAIR TO PLAY.

### TR1d LANDED (09:57): cee8e3d999, marker 'TR1d: the box named' server=1 in stage-bin

- Chain btlkcaqb6 (fired 09:35:28 on w6k-staged +300 s): E free 30.7 GB, patch applied (crlf=False), check ok, pin (W14-j's, extended with the boxed cases) ok, commit cee8e3d999 (09:46), both halves built, staged 09:56:49. Two instruments: (1) `basin_verdict(reached_road, cells, cap, boxed)` now reads "boxed" when no road was reached and a reached cell lies on the +-24 xy / +-8 z box's edge (RIM_BOX_XY / RIM_BOX_Z), so a "closed" field the size of the box (b1 today: 1,596 and 2,092 cells; the general store hauler's 1,926 at 07:55) is told apart from a walled one (b2 today: 616 cells at (7710,6295,181), which the mover left by a two-block drop to the quarry floor); the rim line prints `boxed=` and its text ends "(W14-j; TR1d: the box named)". (2) The TR1c daily probe asks the live router a third pair: road -> the first non-private stockpile's floor centre (the general store), beside the upstairs bed and the ground bed. Bars: the store reads closed+agree-cut (its door is the cut) or boxed+agree-path (the box was the census's limit). Falsifier bhf7gk2l8 fires +90 s (plants `(false, _, _) if boxed => "open"`); reader wait-probe-tr1d by6gte9yr reads the first probe and the first rims after the arms board it. E2-v's chain bo6392uuh fires 10:01:49.
- Falsifier bhf7gk2l8 (09:58-10:02): planted `(false, _, _) if boxed => "open"` (a boxed field named open): pin the_basin_names_its_rim went RED (good) at bastion_jobs.rs:59050; restored 0 dirty files. Shipper bkbmp8an9 shipped cee8e3d999 to lab-bin at 09:57:12: THE PAIR TO PLAY. Both arms boarded W6-K b1018162 (carrying A1) at 09:49/09:55; TR1d boards at their next restarts (~10:25-10:30).

### W6-J's first nights (09:45, both arms, day 1 hour 2): 43 of 50 in bed on both arms with to_bed 0, standing at hour 1 = 0, the seven awake all on the night watch; UNCLIMBED 0 with no occasion (b1 had no built anchor; b2 one nobody over-reached toward)

- wait-ladder-w6j bc1rynlcy. b1 (3b9661d2 since 08:57): NIGHT hours 21..2 in_bed 1/8/33/42/41/43 of 49-50, to_bed 5/24/9/1/2/0; STANDING at hour 1: 0; WAITS 0; UNCLIMBED 0; built anchors none; never arrived 1 of 44 sent (colonist 73, upstairs bed (7724,6369,186), 16 fill-exhausts, 0 rim lines); PIT stalls 0. Hour-1 census: watch=7 working=0 idle=0 untired=0 rest_mean 0.95; far = (84 at 72 blocks, 78 at 48 blocks, both Traveling with stuck 0.03). b2 (3b9661d2 since 09:05): in_bed 0/6/35/42/43/43, to_bed 1/26/5/0/0/0; STANDING 0; UNCLIMBED 0 with one built anchor at (7655,6354,183); never arrived 0 of 48; upstairs beds 0; PIT stalls 0; watch=7; far=[]; rest_mean 0.98.
- Disposition: bar (1) standing at built anchors 0: MET on both arms; bar (2) UNCLIMBED >= 2: NOT EVIDENCED -- the witness had no occasion (no over-reaching walker with a built anchor within 24 blocks; with Q1 and A1 no ladder is planned through a house and none is needed), so W6-J's live arm has not fired and stands as a safety net; bar (3) to_bed at hour 1 <= 8: MET (2 and 0). The night accounts for everyone on both arms: 43 in bed + 7 on watch = 50; nobody idle, working or standing. b1's one never-arrived (colonist 73, an upstairs bed, 16 fill-exhausts) is the far class W6-K names from its next night.
...
- Falsifier bbvp3pwy5 (09:06-09:09): planted `matches!(kind, Mine | Ladder) || true` (every kind admitted to the dig mask): pin the_access_planner_spares_the_houses went RED (good) at bastion_jobs.rs:58928; restored 0 dirty files. Shipper bkbmp8an9 shipped 723205a947 to lab-bin at 09:04:55: THE PAIR TO PLAY.

### The Q1 reader's b2 block (08:58, day 1 hour 3): NIGHT in_bed 42/41/42/43 at hours 0-3 of 50 with to_bed 0/1/0/0 -- 43 OF 50 IN BED AND NOBODY WALKING, the best night on either arm; mine completions 140 with 0 inside houses; STAIR 56/56; Bed cut 0/116; pit stalls 0 (bobs 12); anchors plan 0 built 0. Q1 PASSED on both arms; E2-u's b2 block matches the 08:45 read (CARGO 0, empties 5 of 124, double-claimed 21 of 226).

### The readers' b1 blocks (08:54, day 1 hour 3, pair 16a73b56b2): Q1 -- 169 mine completions, 0 in houses, STAIR 56/56, Bed cut 0/115, stalls 0, anchors 0, NIGHT in_bed 42 of 50 at hours 1-3 with to_bed 2; E2-u -- CARGO KEPT 0, DELIVERY OF NOTHING 6 of 66 deliveries, double-claimed 29 of 170, night shelf empty with a gone load 0

- wait-quarry-q1 (b1): QUARRY SPARES 13 lines (the first pass at boot with 21 houses loaded spared 1,430; the full-town passes spared 984-987; emitted 0 at every witnessed pass); MINE completions 169, inside houses 0; CENSUS day 1 STAIR beds 115 upstairs 56 connected 56 cut 0, SITE REACH cut 2 (cut_open 2) Bed (115, 0) Designated (130, 0); PIT stalls 0 bobs 5; anchors plan 0 built 0; NIGHT in_bed 6/36/40/42/42/42 at hours 22-3, to_bed 31/7/4/2/2/2.
- wait-cargo-e2u (b1): CARGO KEPT 0; DELIVERY OF NOTHING 6; ate-from-pack 19; haul delivered 66 (amount=0: 6); double-claimed 29 of 170; night shelf empty with gone>=1: 0 (E2-u's one bar that passed: no sleeper's shelf read empty over an amount-0 load tonight; the empties were other hauls); starving 1. E2-u's disposition as at 08:45: the rule is right and rarely reached; the empties are E2-v's (the re-issued haul).

### Q1's second replicate, b2 day 1 (08:51): STAIR 56 of 56 connected, Bed cut 0 of 116, mining 140 with 0 inside houses, anchors 0, stalls 0, standing 0 -- and at hour 0 IN_BED 42 OF 50 WITH TO_BED 0, the best night on either arm. Q1 PASSES EVERY BAR ON BOTH ARMS.

- b2 day-1 censuses (boot 08:12 on 16a73b56b2): STAIR beds 116 upstairs 56 connected 56 cut 0 (54/2 the day before); SITE REACH cut 44 (cut_open 44), Bed (116, 0), Designated (51, 34) -- the south farm's one-way ground, unchanged and unrelated to mining. Mine completions 140, inside houses 0 (97 of 144 the day before). Access anchors (plan) 0; THE STANDING SLEEPER lines at hour 1: 0; THE BOB IS A STALL 0. NIGHT: in_bed 4/33/42 at hours 22-0 of 49-50, to_bed 23/8/0. On the same arm the previous nights read 32-37 in bed with 6-13 walking.
- Disposition of Q1: PASSED, two replicates (b1 40/50 with 4 walking; b2 42/50 with 0) -- in-house mining 80-97 -> 0, the loopers' beds 2 cut -> 0 on both arms, the pit undug, no ladder planned, the over-reach steer idle. The row's commit stands as written; A1 (the access planner's mask) lands next as the second half of the class (moot tonight -- with no house hollowed the planner had nothing to shaft -- but right by construction, and its read will show 0 anchors inside houses either way).

### Q1's day-1 read on b1 (08:47, pair 16a73b56b2): THE LOOPERS' BEDS ARE REACHABLE AGAIN, THE PIT IS UNDUG, NO LADDER IS PLANNED -- STAIR CENSUS 56 of 56 connected (54 before), Bed cut 0 of 115 (2 before), anchors 0, WAITS 0, stalls 0, the night 40 of 50 in bed with 4 walking at hour 0

- Day-1 censuses (b1, boot 08:22 on Q1): STAIR beds 115 upstairs 56 connected 56 cut 0; SITE REACH reached 153,294, cap false, sites cut 2 (cut_open 2; neither a bed), Bed (115, 0), Designated (130, 0), Stockpile (92, 0); THE SITE IS CUT OFF kind=Bed 0. The daily probe: upstairs agree-path in 5 polls, ground in 3. Mining that day: 124+ completions, 0 inside houses (08:25). Access anchors registered (plan): 0 -- with no house hollowed, the access planner laid no shaft and no ladder; THE OVER-REACH STEER WAITS: 0 (no anchor to decline); THE BOB IS A STALL: 0, THE BODY BOBS 5. NIGHT: in_bed 6/36/40 at hours 22-0 of 49-50, to_bed 31/7/4 -- b1's best night on record (its earlier nights: 34-39 in bed, 6-15 walking).
- Against Q1's bars: in-house mine completions <= 5 PASS (0); day-1 STAIR cut 0 PASS (from 2); Bed cut 0 PASS (from 2); the pit's stalls 0 PASS; mine completions >= 30 PASS (124 by hour 10). Every registered arm holds on the first replicate; b2's day 1 (~09:00) is the second. The row's consequence reaches further than its bars: the ladder plans, the over-reach steers and the standing sleepers of the last twelve hours were downstream of the hollowed houses. Ben's judgement on the two-block rises still stands for the doors and stairs the worldgen itself makes, but the loopers' house was never one of them: the quarry cut it.

### E2-v THE HAUL HAS NO CARGO -- registered 08:45 (before the binary exists), queued behind TR1d: E2-u's first day read (partial, hour 19): THE CARGO GUARD NEVER FIRED AND THE DELIVERIES OF NOTHING PERSIST (3 and 5) -- their cause is the re-issued haul; W6-J's falsifier RED

- E2-u's day on both arms (pair 16a73b56b2, day 0 hour 19; producer: the E2-u witnesses and the claim/release/deposit lines): THE CARGO IS NOT A RATION 0 and 0; ate-from-pack 12 and 16; THE DELIVERY OF NOTHING 3 (b1: jobs 631 meat, 623 meat, 589 mushroom) and 5 (b2: 520 lettuce, 491 mushroom, 584 curry, 572 a pickaxe, 526 bird meat); haul delivered 62 and 124 with amount=0 3 and 5; material jobs claimed twice 27 of 170 and 21 of 226. Against E2-u's registration: CARGO lines >= 2: FAIL (0 -- no hauler was hungry with a load in hand today; the guard's arm was not reached); DELIVERY OF NOTHING <= 1: FAIL (3, 5). The registered falsification "empties >= 3 with CARGO present: the double claim empties the hand" fired in its spirit: the cargo path (144's supper) was one instance; the common path is below. E2-u stays (a correct rule with a rare trigger); its bar moves to E2-v.
- THE TRACE (b2, job 520): SUPPER LOAD MINTED 12:20:37 (one lettuce for eater 130); claimed by 940 at 12:22:11 (carried 0), arrived at the item (7698,6447,182) 12:22:13 -- picked up; 12:23:54 "UNREACHABLE PROVEN -- job benched off the board (silent writer named) reason=stuck-timeout release" and "job unreachable -- claim released" with 940 at (7769,6362,182), three cells from the shelf (7766,6353,181); re-claimed by 66 at 12:24:41 (carried 0); "haul deposited job=520 dropped=0" and THE DELIVERY OF NOTHING at 12:26:22. The lettuce stayed in 940's pack. The claim filter (~51026) never asks whether the Haul's item still exists where the job says.
- E2-v (fix-e2v.py/.msg; chain bo6392uuh on tr1d-staged; falsifier bwj4u1609 plants every claim admitted; reader wait-cargo2-e2v b7oryokmf): haul_claim_admitted(carried, cargo_in_world) = carried || cargo_in_world; in the claim filter after the guard door, a JobKind::Haul whose item entity (id_maps.uid_entity) is no pickup in the world and whose required item the claimant does not carry takes priority 0 and prints THE HAUL HAS NO CARGO (job, colonist, cargo, required_item, refused); the unclaimed load falls to the existing sweep. Pin the_haul_has_no_cargo. Dry tree (HEAD 3b9661d2d1 + a1 + w6k + tr1d + e2v): clean. Queue: A1 (building) -> W6-K -> TR1d -> E2-v.
- PREDICTION (registered before the binary exists; the first arm-day): DELIVERY OF NOTHING <= 1 per arm-day (from 3 and 5); NO CARGO >= 2; deliveries with amount > 0 >= 50; NIGHT SHELF EMPTY with a delivered-and-gone load 0. FALSIFIED if empties stay >= 3 with NO CARGO present (a third path: the deposit fails with the load in hand) or deliveries fall under 30 (the lookup refuses live loads). Rejected: dropping the carried load at release in this row (E2-w after the read); refusing the completion on dropped == 0. NOT evidenced: loads left in packs; the stuck release three cells from the shelf (W17-c's stand class).
- W6-J's falsifier bxmwj244j planted the ladder climbable: "went RED (good)".

### W6-J LANDED (08:39): 3b9661d2d1, marker 'LADDER NOBODY CLIMBS DOES NOT STAGE' server=1 in stage-bin

- Chain bmkm7ngjx (the repaired one): check ok 08:21, pin the_ladder_nobody_climbs_does_not_stage, commit 3b9661d2d1, both halves built, staged 08:38:57. Falsifier bxmwj244j fires +90 s (plants the ladder climbable); reader wait-ladder-w6j bc1rynlcy reads the first night. A1's chain fires 08:43:57. Both arms are at day 0 hour 18 on Q1: their Q1 nights (~08:45-09:05) read the pit and W6-I's third replicate; they board W6-J at their hour-6 restarts (~09:10-09:15).

### Q1's second replicate on b1 (08:25, day 0 hour 10, pair 16a73b56b2): mine completions 124, INSIDE HOUSES 0 (80 of 111 and 82 of 126 on b1's earlier days); the boot pass emitted 96 outside the houses; STATE demand 24, supply 156, pending 64, quota 0.

### Q1's day on b2 (08:24, day 0 hour 13, pair 16a73b56b2): mine completions 140, INSIDE HOUSES 0 (yesterday 144 with 97 inside); the generator's boot pass emitted 96 outside the houses; quota 0 since (supply 205 >= demand 8): the quarry is fed, not starved

- Producer: the house/mine join on b2's log; AUTON-1 STATE demand 8, supply 205, pending_mine 0, quota 0, cap 98, anchor_sources 216; THE QUARRY SPARES THE HOUSES: 10 passes emitted 0, one pass (the boot) emitted 96 with spared 987. Yesterday's run on the same arm: 141 completions (pending 1, quota 0 at the end). Against the bars: in-house <= 5 PASS (0); mine completions >= 30 PASS (140); the falsification arm "under 10: starved" did not fire -- the quarry moved outward and dug the same volume. The day-1 censuses (Bed cut, STAIR cut; ~08:50) and the night (the pit) complete the read; b1 boarded Q1 at 08:22 (its boot pass: spared 987, emitted 0 with the adopted plots' jobs pending) for the second replicate.

### Rim tallies at 08:20 (both arms' W14-j days): b1 12 lines (open 8, closed 4), b2 11 (open 9, closed 2); the fixture-standing class (cells 0) is 1 of 23 (uid 108 on b1): no row for it now; the closed lines are the store-interior class TR1d reads as boxed or walled.

### Q1's first hours on b2 (08:18, day 0 hour 10): 139 MINE COMPLETIONS, 0 INSIDE HOUSES (97 of 144 the day before on the same arm) -- the quarry spares the houses and the mine lane is not starved; W6-I's second replicate (b2's W14-j night): 35-37 of 50 in bed, to_bed 6-7, two standers at the built anchor

- Producer: the join of the ADOPT-IN-PLACE house lines with the "job completed ... Designated(Mine)" lines on b2's live log at day 0 hour 10 (pair 16a73b56b2): mine completions 139, inside adopted house footprints 0. THE QUARRY SPARES THE HOUSES x11 by then, spared 984-987 per slab, emitted 0 at every pass (quota 0 with pending_mine 98 at boot: the adopted plots' own mine jobs feed the lane; the generator has not needed to mint). The day-1 census (~08:50) reads the bed cut; wait-quarry-q1 prints the whole day. Against the bars so far: in-house <= 5 (0: PASS at hour 10); mine completions >= 30 (139: PASS).
- b2's W14-j night (a8022d7214's fix in the pair; archive prev-server-081253.log; the second W6-I replicate): in_bed 35/36/37/37 at hours 2-5, to_bed 7/6/6/6; WAITS 13, LADDER BUILT 2, stalls 0; STANDING at hour 1: 2 -- 22 at (7706,6310,183) (the built anchor; push bridge-probe, clock 0.13) and 58 at (7697,6303,181) (beside anchor (7698,6303); chaser-settle). The built-anchor class holds until W6-J lands (building now).

### Q1 LANDED (08:12): 16a73b56b2, marker 'THE QUARRY SPARES THE HOUSES' server=1 in stage-bin; shipped 08:12:35; b2 boards it at once

- First witness (b2 boot on 16a73b56, 08:13, zones 100): THE QUARRY SPARES THE HOUSES x8 in the first minutes -- houses 58, spared 987/987/987/984 per pass (of the 2,401 cells scanned per slab), emitted 0 at slabs 184, 183, 182, 181 (the boot's quota; the day's mine completions are the read: bar >= 30, and the falsification arm "under 10: the margin starves the quarry"). Boot census Bed (116, 0).
- Chain bb0mxymv3: check ok 07:56, pin the_quarry_spares_the_houses, commit 16a73b56b2, both halves built, staged 08:12:21. Falsifier bd9bupe8f planted the rule false: "went RED (good)", restored clean 08:17; reader wait-quarry-q1 bjfuvoi4z reads the first day (spared per pass, in-house mine completions, the day-1 bed cut, the pit). b2's daily cascade (cycle 6, 08:12) restarted b2 onto 16a73b56 after its W14-j night; b1 (day 1 hour 1) boards at its hour-6 restart (~08:25). W6-J's chain fires 08:17:21.

### Chain repair (07:57): W6-J, W6-K and TR1d's chains were derived with `sed s/a1/…/g`, which rewrote the scratchpad's session-id path ("a134e3b1" -> "w6j34e3b1"): their pid writes failed and they polled a marker under a directory that does not exist. Stopped by task id, paths repaired, every script audited (all others carry the right id), relaunched: W6-J bmkm7ngjx (on q1-staged), W6-K b7k23gye0 (on a1-staged), TR1d btlkcaqb6 (on w6k-staged); pid files written, holding. Memory filed: a-row-id-can-be-a-substring-of-the-session-path.

### W14-j's first rim lines (07:48, b1 ten, b2 one, pair b68d3b1532 day 0): THE SOUTH FARM IS OPEN FROM ITS FLOOR (a two-high rim the router jumps; the way in is a drop it does not take), A HAULER INSIDE THE GENERAL STORE READS CLOSED (1,926 cells, no road, a flat map), and one body stands on a fixture (0 cells) -- TR1d THE PROBE ASKS THE STORE; THE RIM NAMES ITS BOX registered 07:50 and queued behind W6-K; E2-u's falsifier RED

- Verdicts: b1 open 7, closed 3; b2 open 1. uid 54 from (7666,6453,181) -> (7652,6466,181): open, 937 cells, max_z 184, road (7657,6435,180), rising 1; map west "222" (a two-high rim: the router's jump takes it), a '#' column, east "1 2 ^" (the terrace rises to a wall). The south farm is OPEN from its floor for the router's own steps: the road's census (TR1b) could not descend into it -- the router falls one and jumps two, so the way in is a drop and the way out a jump -- and the walker's exhausts are at its TARGET (a farm cell). uid 120 from (7673,6359,182) -> (7703,6346,181): "closed", 1,926 cells, max_z 190, no road, rising 0, a flat map with a '#' row -- the general store's floor (7657-7680, 6353-6378) and its roof; whether the store's door is a two-block rise the router refuses (the loopers' class) or the +-24 box ended the walk before a road, basin_verdict cannot say. uid 108 from (7681,6393,182): "closed", 0 cells, the map all 'a' -- a body standing on a cell the router does not walk (a fixture; W17-c's class). uid 109 from (7690,6296,180): open, 2,004 cells, road (7672,6301,181). b2's 51 from (7713,6142,181): open, 1,432 cells.
- What it decides: the basin's walkers exhaust at their targets (farm work cells the router does not stand on, upstairs cells), not at their start; the store's floor is the open question -- every hauler walks into it and the daytime flood's top ends lie inside it. Against W14-j's registration ("closed, rising 0, rim '^' or '2' beside '#'" for the terrace): the terrace read OPEN with rising 1 -- the registered arm "open: the road is within the router's reach from the feet; the fill budget against an open terrace is the read" fired for the farm. The instrument is validated (four shapes, three classes, one named blind spot: the box).
- TR1d (fix-tr1d.py/.msg; chain bis000spv on w6k-staged; falsifier bhf7gk2l8 plants boxed named open; reader wait-probe-tr1d by6gte9yr): basin_verdict(reached_road, cells, cap, boxed) names "boxed" when no road was reached and a reached cell lies on the box's edge; the rim line prints boxed; the daily probe adds ("store", the first general store's floor centre). Pin the_basin_names_its_rim extended. Marker 'TR1d: the box named' (appended to the rim message). Dry tree (HEAD aa6538ae0b + q1 + w6j + a1 + w6k + tr1d): clean. Queue: Q1 (running) -> W6-J -> A1 -> W6-K -> TR1d.
- PREDICTION (registered before the binary exists): rim lines from inside the store read closed with boxed=false and the store probe agree-cut or mirror-only (the door cuts the store's floor: the flood's source), OR boxed and agree-path (the box; the flood's from= cells inside the store are a budget question). Falsified as an instrument if a line reads open with rising 0 from a cell ringed by '^'.
- E2-u's falsifier bcy3qflip planted the cargo admitted: "went RED (good)".

### E2-u LANDED (07:47): aa6538ae0b, marker 'THE CARGO IS NOT A RATION' server=1 in stage-bin

- Chain bv8rflenl: check ok 07:30, pin the_cargo_is_not_a_ration, commit aa6538ae0b, both halves built, staged 07:47:09. Falsifier bcy3qflip fires +90 s (plants the cargo admitted); reader wait-cargo-e2u byqt729ud reads the first day (cargo kept, deliveries of nothing). Q1's chain fires 07:52:09.

### W6-K THE WALKING SLEEPER NAMES ITS STATE -- registered 07:42 (before the binary exists), queued behind A1

- The gap: b1's W6-I night held 14-15 to_bed at hours 0-2 with 3 standing; the other eleven moved and never arrived (42 declined three plan-steers with its feet advancing 7732 -> 7729 -> 7741 toward its bed at (7749,6332,186); 59 and 787 exhausted five fills each from their feet; b2's 65 six plus three CLIMB BANNED). W6-H records standers only.
- W6-K (fix-w6k.py/.msg; chain baqosccex on a1-staged; falsifier blxzupvg1 plants every speed as walking; reader wait-walking-w6k beud4mi5p): sleeper_walks(speed) = !sleeper_stands; a moving ToBed body is recorded (up to 12) with dist, feet, bed, speed, clock, route (len, idx) + age, last push, pending pump search, chaser exhausts; the census line gains walking=[..]; hour-1 lines THE WALKING SLEEPER NAMES ITS STATE. Pin the_walking_sleeper_names_its_state. Dry tree (HEAD aa6538ae0b + q1 + w6j + a1 + w6k): clean. Queue: E2-u (building) -> Q1 -> W6-J -> A1 -> W6-K.
- PREDICTION (registered before the binary exists; the first arm-night with to_bed >= 8 at hour 1): the entries read route=None with pending=true or exh >= 1 (the fill exhausts from the walker's start: W14-j's rim line for the uid names the ground) OR route=Some with push bridge-refused-rock/bridge-hold (a rise on the route the mover refuses: the judgement class). Falsified as a framing if entries read route=Some, push chaser-settle/glide, pending=false, exh=0 and still never arrive (the route ends short of the bed: its last node vs the bed is the next read).

### W6-I's first nights (07:32, both arms on a8022d7214, read at day 1 hour 2): THE PLAN-STEERS ARE DECLINED (WAITS 15-16), b2 improved (standing 8 -> 5, to_bed 10-13 -> 6-8, the pit quiet), b1 got worse (to_bed 14-15) -- ONE COMPLETED LADDER CELL REGISTERS "BUILT" AND STAGES AGAIN; the freed walkers' fills exhaust from their feet. W6-J THE LADDER NOBODY CLIMBS DOES NOT STAGE registered 07:36 and inserted ahead of A1

- b2: WAITS 16, LADDER IS BUILT 1 (anchor (7706,6310,182)), anchors plan 4; NIGHT in_bed 35/35/37 at hours 0-2 of 50, to_bed 6/8/6; never arrived 4 of 42 (19 -> (7574,6386,186); 48 -> (7748,6333,186); 55 -> (7825,6263,181); 65 -> (7700,6309,186)), upstairs 3; PIT stalls 0 (bobs 21); STANDING at hour 1: 5 -- 19 at (7705,6310,181) push bridge-refused-rock clock 3.0; 48 at (7704,6312,181) bridge-hold 0.8; 58 and 63 at (7706,6311,181) bridge-refused-rock 8.9/6.5 (beds (7714,6343,186), (7713,6343,186): THE LOOPERS' TWO) -- all at the BUILT anchor's cell, clocks accruing (the ladder will strike); 55 at (7772,6391,187) chaser-settle 0.03. Loop tallies: STRUCK OUT 11, shun 8, rest preempts 66 for 43 arrivals, 9 colonists re-preempted (the bed loop live; W6-G/H2-p consumers running).
- b1: WAITS 15, LADDER IS BUILT 2 ((7686,6317,182), (7696,6319,182)), plan 6; NIGHT in_bed 29/27/28, to_bed 14/15/14 (from 6-11 on b1's earlier nights); never arrived 7 of 37 (11, 23, 39, 42, 59, 62, 787; upstairs 5); PIT stalls 0; STANDING 3 -- 48 at (7686,6317,183) and 62 at (7685,6317,181): THE BUILT ANCHOR; 39 at (7630,6260,181) (bed (7711,6191,181), ground). Loop: STRUCK OUT 0, shun 0, 33 arrivals of 39 preempts. The declined walkers MOVE (42's feet 7732 -> 7729 -> 7741 toward its bed at (7749,6332,186) across three declines) but do not arrive: 59 and 787 exhausted five fill searches each toward their upstairs beds from their feet, 42 two, 11 one; b2's 65 six plus three CLIMB BANNED.
- Against W6-I's bars: WAITS >= 6 PASS (15-16); to_bed at hour 1 <= 4: FAIL (8 and 15); never-arrived upstairs <= 2: FAIL (3 and 5); the pit's stalls 0: PASS. The registered falsification arm "to_bed stays >= 8 with WAITS present: the trunk has no route to the upstairs bed either" fired for the freed walkers (their fills exhaust from their feet toward beds the road probe reaches in five polls: the START is off the router's reach -- W14-j's rim census, landed 07:21 and boarded at the next restarts, reads it from their feet). And a second, unregistered arm: W6-I's build site marks an anchor built on the FIRST completed Ladder cell, so a one-rung plan stages climbs the mover cannot make -- six of the eight standers tonight stood at built anchors.
- W6-J (fix-w6j.py/.msg; chain bjmarlphh on q1-staged; falsifier bxmwj244j plants the ladder climbable; reader wait-ladder-w6j bc1rynlcy): LADDER_CLIMBABLE = false (the kinematic mover does not climb a ladder; Ben's judgement -- climbing as a skill vs a mover that cannot jump -- is open; no body has climbed one on any arm); anchor_stages(built, climbable) = built && climbable; the built pick is filtered by it; a declined built anchor prints THE LADDER NOBODY CLIMBS DOES NOT STAGE. Pin the_ladder_nobody_climbs_does_not_stage. A1's chain killed by pid file and re-pointed to w6j-staged at +300 s (its 45-minute hold dropped: W6-J changes no mining, so Q1 still reads alone against A1). Queue: E2-u (building) -> Q1 -> W6-J -> A1. Dry tree (HEAD aa6538ae0b = E2-u + q1 + w6j + a1): clean.
- PREDICTION (registered before the binary exists; the first arm-night): standing entries at built anchors 0 (from 2 and 4); UNCLIMBED >= 2 per arm-day; to_bed at hour 1 <= 8 both arms; the never-arrived with upstairs beds >= 3 REMAIN (the fills from their feet: W14-j's rim lines name the ground, TR1c's probe stays agree-path). FALSIFIED if standing at built anchors persists (another steer: the emergency route mount) or to_bed rises past 15 with no standers (the freed walkers circle: W6-K's walking entries next). Rejected: built = the whole ladder (moot while nothing climbs); teaching the mover to climb here (the judgement). NOT evidenced: the freed walkers' ground (W14-j); the moving-not-arriving (W6-K).
...
- A1 (fix-a1.py/.msg; chain bv7prtn24 on q1-staged with sleep 2700; falsifier bbvp3pwy5 plants every kind admitted; reader wait-access-a1 by0nxnewz): access_mask_kind(kind) = Mine | Ladder; JobBoard::access_mask_regions(); both callers use it and print THE ACCESS PLANNER SPARES THE HOUSES (claims, designations, spared, pass). Pin the_access_planner_spares_the_houses. Dry tree (HEAD a8022d7214 + w14j + e2u + q1 + a1): clean. Queue: W14-j (building) -> E2-u -> Q1 -> (45 min) A1.
- PREDICTION (registered before the binary exists; the first arm-day from a fresh boot after Q1's own day): spared >= 100 of the designations per pass; every access anchor registered that day lies outside adopted house footprints (from 4 of 6 and 4 of 4 inside); in-house Mine completions at Q1's level or lower; the planner still emits access for deep mine jobs (proactive descent emitted >= 1 a day) or the mine gate's starved columns stay empty. FALSIFIED if the planner emits nothing all day with deep mine jobs pending (the claims alone give no room: the approach corridor needs a re-cut), or anchors still register inside houses (another registration path).
- Rejected: masking inside plan_access (the callers own the mask); adding Chop; a margin (in_access_mask's +-1 is the claim's own leak; a claim beside a house is the plot's question). NOT evidenced: the shafts' share of the in-house cells (Q1's read); whether a body stranded inside a house can still be rescued (its stair must now start from a claim: the self-rescue lines say).

### W6-H's first read (07:01, b2 day 1 hour 1, pair 8cad9cd1f5): EIGHT SLEEPERS STAND AT ONE CELL, ONE BLOCK FROM THE RUN'S ONLY PLANNED LADDER -- steer away, no route, the chaser's settle as the last push, the clock at one tick; W6-I's falsifier RED

- THE STANDING SLEEPER NAMES ITS STATE x8 (hour 1): 21 (bed (7555,6401,186), 174 blocks), 26 ((7626,6321,186), 75), 31 ((7700,6308,186), 5), 57 ((7724,6368,186), 69), 62 ((7820,6333,186), 124), 66 ((7820,6291,186), 121), 67 ((7832,6272,186), 136), 75 ((7841,6233,186), 158) -- ALL at feet (7699,6303,182), all beds at z 186; clock 0.03 s; reset and best 999,974-1,000,000 (the 1e6 sentinel: no route ever entered); steer=away; route=None; age Some(36155) (a route stamp from 20 game hours ago) or None; push=chaser-settle; pending=false; exh=0; kin=true; at=Empty, under=Rock. The run's access anchors: ONE, (7698,6303,182) -- one block west of the eight. NIGHT: in_bed 36, to_bed 8 at hours 1-3 of 50.
- Against the registration: "every entry reads clock <= 0.05" PASS; "steer=own-feet and route=None" -- the steer reads AWAY: the anchor base is one block off, outside steer_is_at_own_feet's radius, and the last push is the chaser's SETTLE (the stop band at the base, 1.5-2.5 out, "covered by the climb assist's ladder grab + magnetism" per the trunk's own comment) -- the settle writes the body still and the clock stays at one tick under it (the reset is the settle's, not the exemption's: the framing's second arm, "a push and a route", half-right: a push, no route). The instrument did its job in one night: eight bodies, one cell, one plan, no route, no search (pending false, exhausts 0): they never ask the router. W6-I (landed 06:56; b1 boards it ~07:03) removes the steer to planned anchors; its first night is the outcome read.
- W6-I's falsifier b4k3dm396 planted the plan admitted: "went RED (good)".

### W6-I LANDED (06:56): a8022d7214, marker 'OVER-REACH STEER WAITS FOR A BUILT LADDER' server=1 in stage-bin

- Chain b1ihpqajx: check ok 06:37, pin the_over_reach_steer_needs_a_built_ladder, commit a8022d7214, both halves built, staged 06:55:47. Falsifier b4k3dm396 fires +90 s (plants the plan admitted); reader wait-anchor-w6i bzfmaqour reads the first night (WAITS lines, to_bed, the never-arrived, the pit). W14-j's chain fires 07:00:47. b1 (TR1c, day 1 hour 3) boards it at its hour-6 restart (~07:03): the first W6-I night ~07:40-08:00.

### Third replicates (06:48, b1 day 1 on TR1c 43fe69bdd7): probe agree-path (5 and 3 polls) again; STAIR cut 2 and Bed cut 2 again (the same house); in-house mine completions 82 of 126 (three arm-days: 80 of 111, 97 of 144, 82 of 126); b1's night: in_bed 31-32 of 48-49 at hours 23-0, to_bed 11 (the anchor class), stalls 0. SITE REACH day 1: reached 153,036, cut 9, cut_open 6, Designated 153/0 (b1's job set again holds no south-farm work).

### Q1 THE QUARRY SPARES THE HOUSES -- registered 06:40 (before the binary exists), queued behind E2-u: THE TOWN MINES ITS OWN HOUSES (b1: 80 of 111 mine completions inside house footprints; b2: 97 of 144) -- the root of the pit, the loopers' cut beds, and the ladders planned through houses; W6-H's falsifier RED

- THE TRACE (b1, pair 4232b3ce42, day 0; producer: the ADOPT-IN-PLACE house lines joined on x,y with the "job completed ... Designated(Mine)" lines): 111 Mine completions, 80 inside adopted house footprints: (7696-7719, 6336-6347) 27 cells at z 181-184 -- the loopers' house, whose beds (7713-7714, 6343, 186) read cut 0 at boot and 2 at day 1 on both arms; jobs 104, 353, 385 completed at (7713-7714, 6344, 183-184) at 09:42, the blocks under and beside the upstairs floor; (7588-7605, 6354-6365) 19 cells at z 185 (the upper floor); (7720-7737, 6336-6353) 18; (7690-7707, 6300-6317) 12 at z 181-182 -- the pit's house: the pit at (7700,6303) is its mined ground floor (column Rock/Rock/Rock/Empty from z 180); (7708-7719, 6348-6359) 4. b2 (the same pair, its day 0): 144 completions, 97 inside houses: 35, 35, 23, 4 in the same houses. The planned ladders W6-I named (anchors (7713,6342), (7698,6303), (7706,6310)) sit inside these houses: the access planner lays escape shafts and ladders where the quarry has hollowed the floors.
- THE MECHANISM (bastion_jobs ~25795, the AUTON-1 mine generator): anchor = quarry_site or the mean of the anchor sources (the town's centre); skip_xy = built cells, plans, stockpiles, farms and the BED CELLS' x,y (not the houses); slab_z = anchor.z + MINE_GEN_Z_TOP (2) - cursor over MINE_GEN_Z_SLABS (9); a 49x49 scan (MINE_GEN_RADIUS 24) mints a Mine job on every exposed Rock/WeakRock with a standable stance. Worldgen houses are Rock; their walls and floors, two above to six below the centre's height, are exposed rock two cells from a bed. The arms boot fresh each day, so the houses are hollowed again every day 0 (the boot census reads whole houses; the day-1 census reads the cuts).
- Q1 (fix-q1.py/.msg; chain bb0mxymv3 on e2u-staged; falsifier bd9bupe8f plants the rule false; reader wait-quarry-q1 bjfuvoi4z): QUARRY_HOUSE_MARGIN 2; quarry_spares(house, xy, margin); the generator gathers the Bed designation regions before its scan and skips every cell inside one plus the margin, counting them; THE QUARRY SPARES THE HOUSES prints houses, spared, emitted, slab_z, pass (8 then powers of two). Pin the_quarry_spares_the_houses. Dry tree (HEAD 8cad9cd1f5 + w6i + w14j + e2u + q1): clean (two anchors re-cut for comment lines in the live file). Queue: W6-I (building) -> W14-j -> E2-u -> Q1.
- PREDICTION (registered before the binary exists; the first arm-day from a fresh boot): spared in the hundreds per pass; Mine completions inside houses <= 5 per arm-day (from 80 and 97); day-1 STAIR CENSUS cut 0 and SITE REACH Bed cut 0 (from 2); THE BOB IS A STALL 0 at (7700,6303); the mine lane still completes >= 30 a day. FALSIFIED if in-house completions stay >= 20 (the access planner's shafts dig the houses: plan_access's mask is the next row), if Bed cut 2 persists with in-house mining 0 (worldgen, not mining: the door row as before), or if mine completions fall under 10 (the margin starves the quarry: the radius grows).
- Rejected: masking the houses in plan_access in the same row (its share is this row's residual); a quarry site away from the town by fiat (taste; quarry_site exists); restoring mined blocks (the arms boot fresh; Ben's world may carry holes: a rebuild row if his log shows them). NOT evidenced: the shafts' share; Ben's world; the loopers' door once its plinth is whole.
- W6-H's falsifier bg9ic5igo planted own-feet named away: "went RED (good)", restored clean.

### TR1c's second probe (06:31, b2 boot on 8cad9cd1f5, zones 100): identical -- upstairs path in 5 polls, ground in 3, agree-path both; and THE LOOPERS' BEDS ARE REACHABLE AT BOOT (cut 0 of 116) BUT CUT BY DAY 1 (2 of 116 on both arms): something built during day 0 cuts that house

- ROUTER PROBE day 0 (b2): upstairs (7546,6414,181) -> (7555,6401,186) "path" polls 5 mirror_ok agree-path; ground -> (7573,6436,181) "path" polls 3 agree-path. Two boots, two arms, the same answer: the router's budget is not the cut.
- SITE REACH CENSUS at b2's boot (zones 100, a whole town): reached 153,314, cap false, sites 212, cut 1 (Designated 3/1), cut_open 1, Bed (116, 0). TR1b's day-1 censuses on both arms read Bed (116, 2) -- the loopers' house (7713-7714,6343,186). So at boot the mirror stands beside those two beds and by the day change it cannot: the town changes during day 0 (Build lane 150-200 works a day; the house's plinth/door/wall gets a block, or the interior set changes as plots are adopted). The door row's first read: the day-0 build/carve lines inside (7696-7716, 6336-6346) on a log that runs day 0 to day 1 (b2's current run, ~07:05).

### W6-H LANDED (06:30): 8cad9cd1f5, marker 'THE STANDING SLEEPER NAMES ITS STATE' server=1 in stage-bin; b2 boards it at once

- Chain b39a2jzq2: check ok 06:12, pin the_standing_sleeper_names_its_state, commit 8cad9cd1f5, both halves built, staged 06:30:15. Falsifier bg9ic5igo fires +90 s (plants own-feet named away); reader wait-standing-w6h bqx0tf87h reads the hour-1 lines. W6-I's chain fires 06:35:15. b2's daily cascade (cycle 3, 06:29) restarted b2 onto 8cad9cd1 after its TR1b night (in_bed 34, to_bed 10 at hours 2-5 of 50: the anchor class, five of the last six arm-nights) -- b2's boot gives TR1c's second probe and its first night W6-H's read (~07:05-07:25).

### TR1b's second replicate (06:19, b2 day 1, pair 4232b3ce42): the same two beds cut (the loopers' house), and THE SOUTH FARM IS A BASIN -- 30 work cells cut from the road, NOT reached even with open jumps

- SITE REACH CENSUS day 1 (b2): seeds 2,318, reached 153,090, cap_hit false, sites 388, cut 41, cut_open 37: Bed (116, 2), Stockpile (92, 1), Designated (51, 30). STAIR CENSUS: 116 beds, 56 upstairs, 54 connected, cut 2. The cut Designated cells: (7661,6440,181), (7653,6441,181), (7658,6441,181), (7652,6442,181), (7658,6442,181), (7620,6471,181), (7623,6471,181), ... and (7692,6320,181) -- reached_with_open_jumps=FALSE for all: not the interior-jump rule, not the cap (153k of 400k). The south farm terrace (y 6440-6484, the corner (7637-7654, 6439-6442) where 22 and 27 starved at hunger 0.00 on the E2-q night and where seven walkers' fills exhausted) has no route from the road for the router's own step set, in either direction; the mover drops bodies into it (W18-b2's Open verdict) and nothing climbs out. cut_open 37 = the 30 farm cells + others: the door-cut beds are the only sites the interior rule alone cuts.
- Two replicates agree on the beds (2 of 116, the same house) and b2 adds the terrace (b1's day-1 read had Designated 189/0: b1's job set that day held no south-farm work). W14-j (queued) prints the terrace's rim from a walker's feet; the fix row is the plot's step or ramp at the terrace edge (THE TOWN LAYS OUT PROPER PLOTS; Ben's judgement covers the same class as the plinth door) or a drop verdict that asks for a way up the router takes.
- b2's night on TR1b (hours 22-0): in_bed 2/27/30 of 49-50, to_bed 31/13/11 -- the anchor class again (W6-I builds after W6-H).

### TR1c's first probe (06:14, b1 boot on 43fe69bdd7, zones 100): THE ROUTER FINDS THE UPSTAIRS BED FROM THE ROAD IN FIVE POLLS -- agree-path on both pairs; the budget is not the cut; the registered "path" arm fired: the walkers' START is the question, and W6-I has named it

- ROUTER PROBE day 0: label upstairs, seed (7546,6414,181) -> bed (7555,6401,186): outcome "path", polls 5, closest None, mirror_ok true (4,222 cells), agreement agree-path. Label ground, seed (7546,6414,181) -> bed (7573,6436,181): "path", polls 3, mirror_ok true (4,364 cells), agree-path. (The boot's ASSIGNMENT SUMMARY read zones=100: a full town this time.)
- Against the registration: "the ground control reads agree-path with polls <= 5" PASS; "the upstairs probe reads agree-cut or mirror-only budget-exhausted" -- NEITHER: it reads path, the arm registered as "falsified as an instrument ... then the walker's START, not the bed, is off the reach". With TR1b's full-town read (114 of 116 beds reachable) and the anchor-steer finding (W6-I), the START is the answer: walkers to upstairs beds within 24 blocks of a planned ladder never ask the router; those who do ask reach. The chaser floods that remain (b2: 69 -> (7715,6343,187) x18) target the one door-cut house. Disposition: TR1c did its job in one boot; the daily probe keeps running as the standing check that the road-to-upstairs search stays under its budget (a later budget regression would read pending-at-cap or budget-exhausted here first).
- What remains for the door row (Ben's judgement): one house, two beds, and the chaser's floods at it; TR1c's first upstairs bed is not that house (the sort by x picks (7555,6401,186)); a probe aimed at the cut house would read agree-cut -- a one-line follow-up if the judgement needs the router's own word on that door.

### W18-j on b1's TR1b night (06:13, wait-stall-w18j): a NULL -- no stall, no hold, five bob lines with max bobs 1; colonist 15 stood at the pit cell (7699,6303,182) with the clock at one tick and no bob. b1's night table: in_bed 40/39/39/39/39 at hours 0-4 of 50, to_bed 4/6/6/6/6 (a good night: the standing six include the pit's stander).

### TR1b READ ON THE FULL TOWN (06:04, b1 day 1, pair 4232b3ce42): THE TOWN IS CONNECTED FOR THE ROUTER'S OWN RULES EXCEPT ONE HOUSE -- Bed cut 2 of 116, both in the loopers' house, both reached with open jumps: THE DOOR IS THE CUT THERE, and nowhere else; TR1c LANDED (06:03) 43fe69bdd7

- SITE REACH CENSUS day 1: seeds 2,318, reached 153,036, cap_hit FALSE (the 400k cap covers the town: the 150k "cut 34" of TR1's day 1 was the cap), sites 600, cut 5, cut_open 2, reached_open 154,034; kinds: Bed (116, 2), Cook (58, 0), Craft (3, 0), DepositRun (1, 0), Designated (189, 0), Guard (5, 0), Haul (54, 0), Recreate (27, 1), RestAt (43, 0), Shelter (11, 0), Stockpile (92, 1), TradeMission (1, 1). THE SITE IS CUT OFF x5: Bed (7713,6343,186) and (7714,6343,186) reached_with_open_jumps=TRUE (Air above); Stockpile x1 open=true; Recreate x1 open=false; TradeMission x1 open=false. STAIR CENSUS day 1: beds 116, upstairs 56, connected 54, cut 2 (the same two, from their own floor with the interior rule).
- Against the bars: "Bed cut >= 12 with the live rule, cut_open < cut, cap_hit false" -- cut 2 (FAIL as registered), cut_open 0 for beds (PASS), cap_hit false (PASS). The registered falsification arm ("Bed cut stays 0 -> the A* budget") did not fire either: the mirror with the router's own move set, clearance, landing and interior-jump rules stands beside 114 of 116 beds. The two it cannot reach are the loopers' house's upstairs beds (house_min (7696,6336,180); the 228x/113x loop cell (7712-7715, 6341-6344, 186) of the earlier nights; E2-q moved its shelf from z 186 to 181 on b1 at 04:44): a plinth door whose step INTO the interior is a two-block jump the live rule refuses. THE DOOR IS THE CUT for that house, and for that house only; Ben's judgement (plot step/ramp vs a two-block doorway rise in the router) now has one named house and two named beds.
- What this decides: the ten-to-thirteen standing sleepers are NOT a geometry class (114 upstairs beds are reachable from the road with the router's rules); they are the anchor-steer class (W6-I) -- the walkers never asked the router. TR1c's probe (road -> the first upstairs bed) now tests the A* budget on a connected pair: agree-path expected unless the budget/heuristic fails; mirror-only names the budget.
- TR1c: chain boxtor349: check ok 05:46, pin the_router_is_asked_once_a_day, commit 43fe69bdd7, both halves built, marker 'ROUTER PROBE' server=1, staged 06:03:42; shipped to lab-bin 06:03:52 (THE PAIR TO PLAY). Falsifier b3ha9ueu4 planted every pair as agreeing: "went RED (good)", tree restored clean 06:08; waiter wait-probe-tr1c bj3mpp93p keys on 'ROUTER PROBE' at the first boot (b2's daily restart ~06:35 or b1's ~06:20). W6-H's chain fires 06:08:42.

### E2-u THE CARGO IS NOT A RATION -- registered 05:52 (before the binary exists), queued behind W14-j: THE HAULER ATE THE SUPPER IT WAS CARRYING

- The trace of load 642 (b1, d20fbd9088 run; producer: the claim, arrival, ate-from-pack, deposit and delivery lines): minted 08:59 (4 bird meat, house (7600,6402,180), eater 82); hauler 144 claimed 09:06:45 (carried 0) and arrived at the item (7792,6368,182) 09:06:47; 09:07:02 "ate from pack -- no travel, hunger met from the colonist's own rations colonist=144 def=bird_raw hunger 0.19 -> 0.69" and the job released (RELEASE-DIAG class="haul"); 22 re-claimed 09:07:10 (carried 0); 09:07:56 "haul deposited job=642 dropped=0 dest=(7611,6410,186)" + "haul delivered amount=0"; NIGHT SHELF EMPTY x14 for 82. Tallies: amount=0 deliveries 7 of 52 (b1's run), 4 of 105 (b2's); ate-from-pack 18 and 14; material jobs claimed twice 30 of 178. Read in the code: the pack-meal pick (~33560) takes the first FOOD_DEFS item in the inventory, the cargo included; the delivery (~41945) removes the job whatever deposit_all_of dropped.
- E2-u (fix-e2u.py/.msg; chain bv8rflenl on w14j-staged; falsifier bcy3qflip plants the cargo admitted; reader wait-cargo-e2u byqt729ud): ration_admitted(def, cargo) = cargo != Some(def) with cargo = the active job's required_item; the pick skips it; when the cargo was the only food, THE CARGO IS NOT A RATION prints (colonist, cargo, job, kept) and the store meal follows; THE DELIVERY OF NOTHING prints on dropped == 0 (instrument; the completion unchanged). Pin the_cargo_is_not_a_ration. Dry tree (HEAD 43fe69bdd7 = TR1c + w6h + w6i + w14j + e2u): clean. Queue: TR1c (building) -> W6-H -> W6-I -> W14-j -> E2-u.
- PREDICTION (registered before the binary exists; the first arm-day): CARGO lines >= 2 per arm-day; DELIVERY OF NOTHING <= 1 per arm-day (from 7 and 4); NIGHT SHELF EMPTY with gone>=1 for an amount-0 load: 0; ate-from-pack falls but not to zero. FALSIFIED if empties stay >= 3 with CARGO lines present (the double claim empties the hand: the second claimant's pickup is the row), or CARGO 0 while ate-from-pack stays 14-18 (the frame is wrong). Rejected: refusing the completion on dropped == 0 in this row; reserving the cargo in the inventory (engine); a hunger exemption for haulers. NOT evidenced: the double claim; the completion change (E2-v if empties survive).

### E2-r's and H2-p's first nights read (05:46); b2's W18-j night: 33-35 of 50 with to_bed 9-11 and the pit stalling 14 times under 17 holds; and A DELIVERY OF NOTHING -- 7 of 52 hauls on b1's run and 4 of 105 on b2's delivered amount=0 and counted as delivered

- E2-r (read-e2r-first; b1 on d20fbd90 at 05:28, b2 on 60a9e8ec at 05:45 -- both pairs include E2-r): b1 NIGHT SHELF EMPTY 14 lines, all colonist 82 (load 642, gone=1, ledgered); MEALS private-source 0, general-source 0; starving sleepers 1; THE NEIGHBOUR'S SUPPER STAYS ON THE SHELF 20 lines (the need-preempt refused foreign shelves 20 times: the mechanism is live). b2: NIGHT SHELF EMPTY 0, starving 0. The bar "delivered-and-empty shelf lines <= 2 per arm-night" as written FAILS on b1 (14 lines) and PASSES on b2 (0); the 14 lines are ONE shelf checked fourteen times (the earlier nights' 13-15 were also one colonist each).
- WHO EMPTIED 82's SHELF: nobody. Load 642 (4 units of bird meat for 82, house (7600,6402,180)): claimed by 144 at 09:06:45 (carried_amount 0), who arrived at the item at (7792,6368,182); re-claimed by 22 at 09:07:10 (carried_amount 0); "haul deposited job=642 dropped=0 dest=(7611,6410,186)" and "haul delivered job=642 amount=0" at 09:07:56 -- the hauler reached the shelf (upstairs, z 186: E2-q's class) with nothing in hand and the job completed as delivered. On b1's run 7 of 52 "haul delivered" lines carry amount=0 (13%); on b2's current run 4 of 105 (4%). E2-i3b's "GONE means DELIVERED" was a misread for this instance: delivered NOTHING. The supper ledger counted the house fed. Row candidate after the queue: A DELIVERY OF NOTHING IS NOT A DELIVERY (the completion must require the carried amount, or re-fetch; the ledger must not count amount=0) -- FOOD FOR WINTER's arithmetic depends on it.
- H2-p on b2 (read-h2p-first, pair 60a9e8ec night 0): BEDS OUT 0, BACK 0; STRUCK OUT per colonist: 69 x1; SHUNNED 1; PATIENCE 3. NIGHT: in_bed 29/33/33/35/35 at hours 23-3 of 50, to_bed 11/9/11/9/9, watch 8->6 -- the standing class (third of the last four arm-nights). H2-p's bars: strike-outs 1 (PASS), no sleeper > 2 (PASS), in_bed 35 (PASS), to_bed <= 8 (FAIL, 9: the anchor class).
- The pit on b2's W18-j night: THE BOB IS A STALL 14, THE STALL HOLDS ITS CLOCK 17 (stuck_time 0.0 at the holds: the own-feet reset; W6-I's class as read at 05:34).

### W14-i7b's night on b2 (05:44, pair 60a9e8ec = W18-j, day 0 night): the chaser IS the flood on this arm-day -- W14-i7b PASSES its bars here and fails them on b1: the producer differs by arm and day

- b2 (read at day 1 hour 3): CHASER REPEAT CENSUS 17 lines; hour 1: pairs 5, total 44, top 28@(7728,6353,183) x22, 69@(7715,6343,187) x18, 84@(7705,6393,180) x2; THE FLOOD NAMES ITS STANDING WALKER 16 (pushed=false 3 of 16; kinds Haul 7, EatFrom 6, none 3); FLOOD 52 (top ends 22x (7728,6353,198), 18x (7716,6343,186), 3x (7771,6391,189)). The census's top pair (22x at (7728,6353)) matches the tally's top end (22x at the same x,y); total 44 of 52 = 85%: both registered bars PASS on b2. On b1 the same night: chaser 10 of 317 (the pump's fill lane). TWO PRODUCERS, one flood tally: W14-i8 names the fill's pairs, W14-i7b the chaser's; the 05:05 "falsified as framed" stands for b1's day and is withdrawn as a general claim -- the chaser floods on b2 (69's target (7715,6343,187) is the loopers' upstairs bed cell; 28's (7728,6353,183) a work cell). The end z of 198 for a target at 183 is the search's last closest cell fifteen blocks up: a search climbing a roof for a target under it.

### TR1b LANDED (05:37): 4232b3ce42, marker 'cut_open' server=1 in stage-bin; shipped 05:37:41; b1 gets a STANDING daily cascade

- Chain bpebaj5xd: check ok, two pins (the_town_names_its_cut_off_sites; the common pin the_road_reaches_the_plaza_not_the_walled_yard with lab case F), commit 4232b3ce42, both halves built, staged 05:37:30; shipped to lab-bin 05:37:41 (THE PAIR TO PLAY). Falsifier beged1oy2 fires +90 s (plants the landing rule ignored in path.rs). TR1c's chain took tr1b-staged (fires 05:42:30). The waiter wait-reach-tr1b (b055sm2do) keys on 'cut_open=' at the first boot census.
- b1's TR1b boot census (05:40, pair 4232b3ce, day 0): STAIR CENSUS beds 50 upstairs 22 connected 22 cut 0; SITE REACH CENSUS seeds 1,437 reached 68,597 cap_hit FALSE sites 108 cut 0 cut_open 0 reached_open 69,283, kinds Bed 50/0, Designated 22/0, Stockpile 36/0 -- BUT the boot's ASSIGNMENT SUMMARY read zones=39 (the earlier boots read 100) and beds 50 of 116: the day-0 pass ran on a HALF-LOADED town (a race with chunk loading on this restart). SCOPE: 39 zones. Not a read against the bar (Bed cut >= 12 with the live rule). The day-1 census (~06:15) is the read; the reach without the cap on the full town (cap_hit false at 68k of 400k) is the one number that carries: the mirror walks the whole loaded town in 69k cells.
- b1's one-shot cascades never restarted it after the S1-i block (W6-G and E2-i3b readers still waiting at day 1 hour 8): both stopped by their pid files; `wait-daily-b1.sh` (task bwftphpt0; restart-arm1-g1c.sh) now restarts b1 after each full night onto stage-bin's pair when newer -- its first action boards TR1b at once (day 1 hour 8 on W14-i7b).

### W18-j's first live stall (05:34, b2, pair 60a9e8ec14, day 0 hour 19): THE HOLD WORKS, THE RELEASE DOES NOT -- the clock reads 0.0 at every hold: a second per-tick reset, the own-feet exemption fed by the anchor steer's straight-up target (W6-I's class)

- uid 83, job 507 Designated at (7706,6310,184) -- two cells from the planned anchor (7706,6310,182), z 184 from feet 181-183 = over_reach -> the anchor steer. THE BOB IS A STALL at bobs 16 (09:26:41), 17, ..., 23 (stalls 8 by 09:27:08; the stall re-fires because the clock is back under the timeout). THE STALL HOLDS ITS CLOCK x11 (held 1 at 09:26:45 ... 64 by 09:27:21) with stuck_time=0.0 at EVERY hold and hold_until moving (18645, 18800, 19471): the probe's reset was refused as designed, and the clock was already zero -- zeroed by another site between the stall's set (11.0) and the next tick. THE BOB NAMES ITS COLUMN (W18-j's landing fields): cell (7700,6303) z 183 -> 181, column [Rock,Rock,Rock,Empty,Empty,Empty]; landing (7700,6304), landing_column from z 180 = [Rock, Empty, Empty, Empty, Empty, Rock] -- the body drops from the doorstep at 183 into the house's ground room (floor at 181 under a slab at 185); POS-WRITE "mover" lines alternate (7700.9,6304, 183->181) and (7701.4,6304, 186->184): the body cycles through the house's floors around the ladder plan.
- DISPOSITION: W18-j's mechanism is witnessed (held 64, the probe refused) and its outcome bar FAILED (bobs 16 -> 23+ on the same job, no release within 30 s): the registered falsification arm "held > 0 while the bobs climb: another reset site" fired. The other site is named by the steer: at the anchor base the steer is (base.x, base.y, target.z), straight above the body, and the v2 exemption (`steer_at_own_feet -> stuck_time = 0.0`, ~39292) zeroes the clock every tick. The same exemption pins the standing sleepers (their far-list clocks 0.0/0.033). W6-I removes the anchor steer for planned ladders, which removes the vertical steer and with it the exemption's grip, for the pit and the standers alike; W18-j's hold stays as the guard against the probe. If W6-I's night still shows stalls with clock 0.0 at the holds, the exemption itself gets its row (a ToBed or Designated body with a vertical steer and no route is not resting).
- uid 83's exit (05:36 read, wait-stall-w18j): "Personal entry releases the held work job uid=83 job=507" at 09:27:18 -- 37 s after the first stall a HUNGER preempt took the body (it ate from the general store at 09:28:13), then its fills exhausted toward (7771,6391,189) (an upstairs target, 3 repeats). The pit episode lasted under a minute this time, freed by a need, not by W18-j's ladder; the earlier nights' bobbers held RestAt jobs that no need preempts.
- b1's W14-i7b night (read-w14i7b-first, day 1 hour 3): in_bed 33, to_bed 9, watch 7, idle 0 -- the standing class again (the three at the anchor (7686,6317) among them). W14-i7b's own lines by then: CHASER REPEAT CENSUS 12 lines (hour 1: pairs 5, total 10; top 80@(7743,6404,182)x4, 35@(7743,6404,182)x3), THE FLOOD NAMES ITS STANDING WALKER 10 (pushed=false 3 of 10; kinds Haul 7, none 3), FLOOD 317 (top ends 49x (7734,6411,181), 33x (7756,6409,181), 29x (7632,6286,182)). Correction to the 05:05 disposition: the chaser does exhaust, ~10 a day against ~300 fill exhausts; the frame stands (the fill lane is the flood), and the instrument is not blind.

### W6-I THE OVER-REACH STEER NEEDS A BUILT LADDER -- registered 05:33 (before the binary exists), inserted after W6-H and ahead of W14-j: THE STANDING SLEEPERS STAND AT THE BASES OF PLANNED LADDERS

- b1's W14-i7b night (d20fbd9088, night 0, hour 0): in_bed 35, to_bed 8; far = (83, 152, 0.0, (7684,6317,181)), (142, 136, 0.0, (7684,6317,181)), (152, 118, 0.033, (7684,6317,181)) -- the SAME cell as b2's trio (145, 138, 26) the night before; beds (7646,6465,186), (7820,6333,186), (7769,6401,186): all upstairs. Nothing happens at the cell but "access anchor registered (plan)".
- THE ANCHORS (producer: the "access anchor registered" lines): b1 six -- (7686,6317,182), (7696,6319,182), (7713,6342,182), (7705,6346,182), (7698,6303,182), (7706,6310,182) -- all "(plan)", zero "(built)"; b2 four -- (7706,6310), (7686,6317), (7696,6319), (7698,6303) -- all plans. The trio's cell is two from (7686,6317); THE PIT (7700,6303) is two from (7698,6303). Ladder jobs completed: 2 on b1, 1 on b2 (mine access cells elsewhere; no "(built)" registration on either).
- THE MECHANISM (bastion_jobs ~36675-36711): reach = 2 + min(climbing, 1); over_reach = job.pos.z - feet.z > reach; when over_reach, anchor_steer = the nearest access anchor with z in [feet.z-2, job.z+2] within 24 blocks; the steer walks to its base and, within 1.6 of it, turns to (base.x, base.y, target.z) -- straight up; staged_at_anchor bypasses the committed route and the fill. Upstairs beds (z 186) from feet at 181-183 are over_reach by 3-5. The plan-time registration (plan_access ~6486) pushes every Ladder DESIGNATION's base into access_anchors; the build site (~43078) pushes only when no anchor is within two, so a built ladder over a plan is never distinguished. A body told to climb a plan stands with its steer over its own feet; the v2 exemption pins its stuck clock at zero (the far list's 0.0/0.033 s); the ladder never runs. The sleepers who arrive are those the rest preempt found more than 24 blocks from any plan.
- W6-I (fix-w6i.py/.msg; chain b1ihpqajx on w6h-staged; falsifier b4k3dm396 plants the plan admitted; reader wait-anchor-w6i bzfmaqour): board.anchor_built (the build site marks the near anchor built: THE LADDER IS BUILT once per anchor); over_reach_anchor(anchors: &[(anchor, built)], feet, job_z, pos_xy, built_only); the steer takes the built-only pick and, when only a plan would have been chosen, prints THE OVER-REACH STEER WAITS FOR A BUILT LADDER (uid, job_pos, feet, planned; 8 then powers of two) and keeps the route; the mine-descent gate reads plans as before. Pin the_over_reach_steer_needs_a_built_ladder. Queue now: TR1b (building) -> TR1c -> W6-H -> W6-I -> W14-j (W14-j's chain killed by pid file again and re-pointed to w6i-staged; dry tree from HEAD 4232b3ce42 in the new order: clean). Prior art: DF/Banished designated stairs are unpathable until built; RimWorld blueprints are impassable plans.
- PREDICTION (registered before the binary exists; the first arm-night on the pair): WAITS lines >= 6 by hour 1; to_bed at hour 1 <= 4 (from 8-13); never-arrived with upstairs beds <= 2 (from 5 and 11); the pit's stalls 0; W6-H's entries (if any) read a steer other than own-feet. FALSIFIED if to_bed stays >= 8 with WAITS present (the trunk has no route to the upstairs bed either: TR1c decides door vs budget), or WAITS absent while sleepers stand at the anchors (another steer: W6-H names it).
- Rejected: removing the plan-time registration (the mine gate reads it); built = every rung (a completed cell is the evidence at hand); teaching the mover to climb (Ben's judgement). NOT evidenced: a body on a BUILT ladder; the pit's landing (W18-j); the fill for these walkers (bypassed by staged_at_anchor; expected to run now).

### W6-H THE STANDING SLEEPER NAMES ITS STATE -- registered 05:18 (before the binary exists), inserted ahead of W14-j; W18-j LANDED (05:10) 60a9e8ec14, falsifier RED; b2's second E2-q night REPLICATES the upstairs class (11 of 12 never-arrived have z-186 beds) and the standers' stuck clocks read one tick

- W18-j chain bkbbadxmb: check ok 04:52, pin the_stall_holds_its_clock, commit 60a9e8ec14, both halves built, marker 'THE STALL HOLDS ITS CLOCK' server=1, staged 05:10:09; shipped to lab-bin 05:10:25 (THE PAIR TO PLAY). Falsifier bzf8jf6kq planted the hold ignored: "went RED (good)". b2's daily cascade (armed 05:09) restarted b2 onto 60a9e8ec at 05:11 (its first night ~05:45-06:05; wait-stall-w18j reads the first stall). TR1b's chain took w18j-staged and is at its patch (05:15).
- b2's second E2-q night (pair d894e982, day 1 night; read 05:07 by the E2-i3b reader, day-2 block): in_bed 31/32/32/32/32/32 at hours 0-5 of roster 51, to_bed 13 at every hour, watch 6, idle 0; NIGHT SHELF EMPTY 15 (all colonist 25, load 614 gone: E2-r's class); starving 135, 22, 25 at points; MINTED 100. The first E2-q night had read 43 in bed with to_bed 1: the same pair, the same world, 43 then 32 -- the class is intermittent by night, not by pair.
- WHO NEVER ARRIVED (rest need-preempts since hour 21 vs RestAt arrivals): 44 sent, 33 arrived, 12 never: 126, 135, 138, 143, 1438, 145, 21, 23, 26, 59, 78 -> beds at z 186 (UPSTAIRS, eleven of twelve); 83 -> (7680,6391,181) ground. 21 and 78's beds are (7700,6308-6309,186): the house five cells north of the pit at (7700,6303). With b1's 5 of 6: SIXTEEN OF EIGHTEEN never-arrived sleepers have upstairs beds. THE UPSTAIRS BED IS NOT REACHED is the HUMAN HOURS row.
- THE STANDERS' CLOCKS: the far list's third field is a.stuck_time (not speed, as I had read it): b2 hour 2: (145, 177, 0.033, Traveling, (7684,6317,181)), (138, 159, 0.0, Traveling, (7685,6317,181)), (26, 154, 0.0, Traveling, (7685,6317,181)) -- three sleepers at one farm-plot cell for hours 0-5 with the stuck clock at zero or one tick, and NO exhausted searches of their own (the eleven exhausted fills from (7680-7690, 6312-6322) belong to 81 and 58 (DepositRun) and 23 (RestAt, 8 repeats)). b1 hour 1: (23, 148, 0.033, (7720,6365,181)), (57, 123, 0.0, (7699,6303,182): the pit), (69, 121, 0.033, (7700,6303,183): the bobber). A clock at one tick after hours = a reset path zeroes it every tick and the ladder never runs: the pit's class without the bob; thirty other zeroing sites (the v2 `steer_at_own_feet` exemption among them) and nothing names which, nor the steer, the route, the last push.
- W6-H (fix-w6h.py/.msg; chain b39a2jzq2 on tr1c-staged; falsifier bg9ic5igo plants own-feet named away; reader wait-standing-w6h bqx0tf87h keys on the hour-1 lines): STANDING_SPEED 0.05, STANDING_SLEEPERS_MAX 12, sleeper_stands, steer_class (none/own-feet/away), dist_label; in the night census's ToBed arm a body at no x-y speed is recorded with dist, feet, bed, clock, reset, best, steer, route (len, idx) + age, last push, pending pump search, chaser exhausts, kinematic marker, the block at and under the feet; the census line gains standing=[..]; at hour 1 one line per body: THE STANDING SLEEPER NAMES ITS STATE. Pin the_standing_sleeper_names_its_state. Queue now: TR1b (running) -> TR1c -> W6-H -> W14-j (W14-j's chain killed by pid file and re-pointed to w6h-staged; dry tree from HEAD 60a9e8ec14 in the new order: clean).
- PREDICTION (registered before the binary exists; the first arm-night with to_bed >= 6 at hour 1): every entry reads clock <= 0.05 with steer=own-feet and route=None (the no-route sleeper whose steer defaults to its feet: the v2 exemption pins the clock), OR steer=away with push=bridge-probe and a route (the probe's reset without a bob). Falsified as a framing if the clocks rise past 1 s (the threshold or the release is the question) or the speed test catches gliding bodies (kin/push say so). The consumer follows the class: own-feet + no route -> the exemption must not pin a route-less ToBed body; bridge-probe -> W18-j's hold extends to a standing body.
- Rejected: the far list alone; a stuck-clock fix before the reset is named; a per-tick line. NOT evidenced: which reset; why the trunk hands upstairs beds no route (TR1b/TR1c); the pit's landing (W18-j).

### W14-i7b's first day on b1 (05:05, pair d20fbd9088, boot 04:51, read at day 0 hour 13): FALSIFIED AS FRAMED -- the chaser exhausted nothing; the whole-town flood is the pump's FILL lane, and W14-i8 already names it per pair

- Counts (producer: the raw log): LONGEST-EXHAUST NEIGHBOURHOOD lines 163 (expanded_states 57,994-66,457, median 62,915: whole-town every one); THE FLOOD NAMES ITS STANDING WALKER 0; THE FLOOD NAMES ITS WALKER (W14-i7) 0; CHASER REPEAT CENSUS 0 (gated on a non-empty table); THE EXHAUSTED DELIVERY NAMES ITS ASKER (W14-i8, the pump's Fill arm, powers of two per pair) 95; EXHAUST REPEAT CENSUS 4; PUMP CENSUS at tick 11,117: delivered_exhausted 39 per 300 ticks; wall-detour outcomes: path 2, budget_exhausted at tier 0 (not Longest).
- The diag at path.rs:2582 lives in find_path_priced, the search shared by the chaser and the pump; the pump's Fill lane runs Longest searches (bastion_jobs 45630) -- its "512/1024" is the per-tick poll budget, not the state budget. So the 163 whole-town exhausts are the pump's fills for trunk tails, every one already counted per (asker, target) by W14-i8 -- and the chaser, read for every colonist every tick by W14-i7b, exhausted nothing. W14-i7 (16 lines, pushed bodies only) and W14-i7b were built on the frame "the chaser is the whole-town producer"; W14-i8's own note had said the fill lane was "the other" producer. TWO FRAMES: the FLOOD tally (any Longest exhaust) against the chaser's count.
- Disposition: W14-i7b FAILED its bars (top pair, total >= half the day's count) because its producer is empty, not because it is blind; the instrument stays (harmless; the chaser's table fires if the chaser ever floods). The standing walkers are named by W14-i8's `from` (the basin walkers were), and the fill lane's exhausted arm "keeps the trunk tail and strikes nothing" -- a standing walker whose fill exhausts every few seconds is the pump's client, not the chaser's. The consumer question moves to the pump: what a walker does after N exhausted fills from the same feet (W14-j names the ground; the ladder's release is W18-j's). read-w14i7b-first prints its block at day 1 hour 3 (expect 0 named).

### H2-p's first night on b1 against its bars (04:47, read-h2p-first; pair b60950b0de, day 0 night): three of four bars PASS; the fourth (to_bed <= 8) FAILS at 10 for the upstairs class, not for bed strike-outs

- REPUTATION: BEDS OUT 1 (bed (7748,6333,186): three W6-F shun lines by colonist 51 within 51 s at 08:18, out at strikes=3; the count matches the events), BEDS BACK 0. STRUCK OUT per colonist: 33 x1 (bar: no sleeper strikes out more than 2 beds: PASS). STRUCK OUT per arm-night 1 (bar <= 8: PASS). in_bed 35 at hour 3 (bar >= 30: PASS). to_bed 10 at hour 3 (bar <= 8: FAIL). PATIENCE (W6-G) 0. SHUNNED 4, held max 1.
- Disposition: the mechanism works as pinned and witnessed (one bed out, town-wide, after three strike-outs; no sleeper struck out twice). It does not touch the ten standing: they never strike out (STRUCK OUT 1 all night) because they never reach a wall to be suspended at -- they stand with upstairs beds (five of the six never-arrived below) or at the pit. H2-p is a consumer for the bed-loop class (intermittent, 3 of 8 arm-nights); the to_bed bar belongs to the upstairs class (TR1b/TR1c) and the pit (W18-j). A second night (b2 boards H2-p or later via the daily cascade) replicates the reputation read.

### E2-q's first nights read (04:44, read-e2q-first): b2 43 of 50 in bed (the best night yet); b1 (on H2-p) 34-35 of 50 with ten to bed all night -- FIVE OF THE SIX WHO NEVER ARRIVED HAVE UPSTAIRS BEDS; SHELF MOVED 1 on b1

- b2 (pair d894e982, boot 03:49; read 04:22 at day 1 hour 3): SHELF MOVED 0; DROP CELL FILTER EMPTIED 10; SITE REACH cut 34 (the cap); NIGHT SHELF EMPTY 14 lines, all colonist 25 (gone=1, load 614 ledgered, bed_z 181: E2-r's class, not on this pair); FLOOD 323 (whole-town 319; top ends 32x (7738,6399,180), 28x (7734,6405,181), 27x (7630,6281,182)); STRIKES terminal_chaser 2, longest_exhausts 8; STALL BLAMED 4, FETCH STALLED 24, ARRIVED 907, STRUCK OUT 0, SHUNNED 0, starving 1 (colonist 25, "Traveling" at 167 blocks, speed 0.033, all night). NIGHT: in_bed 36/40/42/43/43 at hours 23-3 of roster 50, to_bed 3/1/2/1/1, watch 8->6, idle 0-1. The best arm-night on record (43 of 50); the previous b2 night (TR1) read 32: the same code, a 2-3x class -- three replicates.
- b1 (pair b60950b0de = H2-p, boot 04:11; read 04:44 at day 1 hour 3): SHELF MOVED 1: zone 47 from (7715,6343,186) to (7715,6342,181), house_min (7696,6336,180) -- E2-q's placer fired once, for the upstairs shelf in the loopers' house (b1's 03:19 night named (7712,6341,186) 228x; tonight's top exhaust end is 11x (7715,6344,186), the bed beside it). SITE REACH cut 8 (Stockpile 92/3). NIGHT SHELF EMPTY 0. LOOP: other_bans 6 (colonist 69 2x at (7703,6349,181), 33 3x), held_strikes 3, struck_out 1. FLOOD 17. STALL BLAMED 5, FETCH STALLED 30, ARRIVED 900, SHUNNED 4, starving 0. NIGHT: in_bed 29/34/34/34/35 at hours 23-3, to_bed 13/9/11/10/10, watch 7->5, idle 0-1. far at hour 1: colonist 23 (148 blocks, at (7720,6365,181)), 57 (123, at (7699,6303,182): THE PIT), 69 (121, at (7700,6303,183): THE PIT'S BOBBER). feet_z histogram: 181: 34, 186: 10, 183: 3, 180: 2, 182: 1.
- WHO NEVER ARRIVED (derived: rest need-preempts since hour 21 = 42 distinct colonists; RestAt arrivals 36; producer: the need-preempt and arrival lines): 22 -> bed (7596,6358,186); 28 -> (7700,6308,186); 36 -> (7711,6358,181); 52 -> (7724,6368,186); 57 -> (7820,6333,186); 69 -> (7756,6412,186). FIVE OF SIX HAVE UPSTAIRS BEDS (z 186). 28's bed at (7700,6308,186) stands five cells from the pit at (7700,6303); 57 and 69 (beds elsewhere) spent the night at the pit. The upstairs class (THE STAIR IS NOT THE CUT -> the door; TR1b/TR1c decide between the geometry and the router's budget; Ben's plinth-door judgement pending) is the HUMAN HOURS row: ten standing sleepers on two of the last four arm-nights, one on the other two.
- E2-q disposition so far: the mechanism witnessed once (b1) and validated (the shelf moved from z 186 to the house's floor z 181); the bar (>= 6 moves per arm) not met, the registered NOTE branch holds (the reach set reaches nearly every shelf at the cap); the class it serves (the loopers' house) is the upstairs class above.

### W14-i7b LANDED (04:42): d20fbd9088, marker 'THE FLOOD NAMES ITS STANDING WALKER' server=1 in stage-bin

- Chain bkoet1aus: check ok 04:26, pin the_flood_names_its_standing_walker, commit d20fbd9088, both halves built, staged 04:42:51; shipped to lab-bin 04:43:12. Falsifier b3cb05rh8 planted the push required: "went RED (good)", tree restored clean 04:47; the reader read-w14i7b-first bkhhsxhmr waits for an arm to board it (b1's cascade restart at hour 6 of day 1, ~04:50; b2's new daily cascade once the E2-i3b reader exits). W18-j's chain fires at +300 s (~04:48).

### THE PIT REPLICATES ON b1 (04:37, pair b60950b0de, H2-p night 0) -- and the queue is REORDERED (04:42): W18-j ahead of TR1b and TR1c

- b1, day 0 hour 22-23: THE BOB IS A STALL x128 for uid 69 at cell (7700,6303) from 08:36:27 to 08:37:30 (one minute: bobs 16 -> 143), THE BOB NAMES ITS COLUMN at 16/32/64/128: z 183 -> 181, site "bridge-probe", column from z 180 [Rock, Rock, Rock, Empty, Empty, Empty], job 1206 RestAt bed (7756,6412,186), same_job true, ticks_since_stall 210/397/811, clock_found 0.033 s at every one; the BODY BOBS lines carry stuck_time 0.033. The second arm, the second night, the same cell, the same clock. H2-p on b1 so far: BEDS OUT 1 (bed (7748,6333,186), three W6-F shun lines by colonist 51 in 51 s at 08:18 -> out at strikes=3; the count matches the events), STRUCK OUT 1. Night table at hour 23: in_bed 29, to_bed 13, watch 7 of 49 (the read at hour 3 follows via read-h2p-first).
- REORDER: W18-j (the fix) now runs first after W14-i7b: chains W18-j, TR1b and W14-j were killed by their pid files while holding (the W14-i7b stage had not fired), their MARK lines re-pointed (W18-j on w14i7b-staged; TR1b on w18j-staged; TR1c untouched on tr1b-staged; W14-j on tr1c-staged), fix-w18j.py's pin re-anchored on TR1's pin (its old anchor was TR1c's), the dry tree re-run in the new order from HEAD d20fbd9088 (W14-i7b): clean; the three chains relaunched (bkbbadxmb, bpebaj5xd, bgzksao8c). Falsifiers and readers key on their own staged markers: untouched.

### W14-j THE BASIN NAMES ITS RIM -- registered 04:20 (before the binary exists), queued at the end (behind W18-j); E2-r LANDED (04:16) 5e2887b40c, falsifier RED; TWO COLONISTS STOOD STARVING IN THE SOUTH FARM (b2, E2-q night)

- E2-r chain b38dveie2: check, pin the_neighbours_supper_stays_on_the_shelf, commit 5e2887b40c, both halves built, marker 'SUPPER STAYS ON THE SHELF' server=1 in stage-bin, staged 04:16:41; shipped to lab-bin 04:17:05 (THE PAIR TO PLAY). Falsifier b2oy5jcmo planted the hunger pick admitting everything: "went RED (good)", tree restored clean 04:21. W14-i7b's chain bkoet1aus took e2r-staged (at check 04:24).
- THE FINDING (b2, pair d894e982, E2-q night 03:49-04:15; producer: E1-i STARVING COLONISTS, the need-preempt lines, W14-i8's exhausted-asker lines): from tick 27,000 colonist 22 at hunger 0.00 "Traveling" to EatFrom items at (7614,6401), (7638,6481), (7644,6395) -- 28 to 47 blocks off -- with feet inside (7638-7645, 6439-6440, z 182) at 04:04, 04:06, 04:09, 04:11, 04:13; colonist 27 the same at (7648-7654, 6441-6442) from 04:07; both under the E2-s shun/re-pick cycle (live_shuns 3-4, steered 12-14) with no arrival. THE EXHAUSTED DELIVERY NAMES ITS ASKER x31 with from inside (7637-7644, 6439-6440): 13x (7644,6439,182), 5x (7637,6440,182), 3x (7644,6440,182); seven distinct walkers (22, 27, 135, 58, 24, 56, 59; targets 40-55 blocks east at (7694-7699, 6445-6447) and west at (7611,6410)). The same terrace on b2's TR1 run (uid 77, DepositRun, repeats 1-8 from (7650-7651, 6439, 182)) and on b1's E2-q run (uid 66, DepositRun, repeats 3-8 from (7635-7654, 6437-6448, z 180-181)). Tilled cells at (7648-7650, 6439, z 180) beside the corner; TR1's b2 day-1 census had read Designated (7653-7661, 6440-6442, z 181) among 32 cells CUT at y 6442-6484 -- which I attributed to the 150k cap (NOTED as a number without its producer; TR1b's 400k cap tests it).
- THE PLACE IS KNOWN: W18-b's note names "the farm terrace's ledge, whose landing is the whole lower town: three farmers stalled at that edge on their way to supper and slept at hunger 0.00 (b2, dfa366b6db, night 0)"; W18-b2 opened the mover's two-block drop into it (drop_verdict Open at >= 64 standable cells). The router's step set (common::path COLONIST_STEP_DIRS/JUMPS/SCRAMBLES) admits one-block falls, two-block jumps (free three above the feet) and scrambles at reach 3 only; the trunk reach is 2. So the way back up is a jump, and seven walkers say it is not admitted; whether the clearance, the landing (a fence/crop sprite on the rim: W17-c's class), the interior rule or a three-high rim refuses it, nothing names. W14-i7b names the chaser's exhaust, not the ground.
- W14-j (fix-w14j.py/.msg; chain bhb2gbyin on w18j-staged; falsifier baw1mszzw plants closed named open; reader wait-rim-w14j bmtt7ng86 keys on THE BASIN NAMES ITS RIM): at reps == 2 of W14-i8's pair (first 8 then powers of two), from the walker's feet a reach set with the router's own steps (TR1b's live rule) in a +-24 x,y +-8 z box (cap 60,000): basin_verdict open/closed/capped; cells, max_z, the first road cell reached, rising (admitted steps up from the feet), and rim = nine strings of rim_glyph ('.' level, '1' step, '2' jump, '^' wall, 'a'/'b'/'v' down, '#' nothing standable). Pin the_basin_names_its_rim. Dry tree (HEAD 5e2887b40c + w14i7b + tr1b + tr1c + w18j + w14j) clean.
- PREDICTION (registered before the binary exists; the first arm-day on the pair): the terrace walkers read verdict=closed, cells hundreds to low thousands, max_z <= feet+1, rising 0, and a rim whose edge rows show '^' or '2' beside '#'. FALSIFIED as an instrument if they read open (the road is within the router's reach: the fill budget against an open terrace is the read, and W14-i7b's chaser verdict decides) or capped (raise the cap). The fix follows the map: '^' -> a plot step/ramp at the terrace edge (Ben's plinth-door judgement covers it) or a drop verdict that asks for a way up the router takes; '2' beside '#' -> the rim landing carries a sprite the router does not stand on.
- Rejected: the pump's exhaust count as the verdict (a budget, not a wall); the reach from the road (TR1: cannot tell a cap from a wall at the far end); a fix before the map. NOT evidenced: the fix; b1's terrace corner; the E2-s cycle's part.

### W18-j THE STALL HOLDS ITS CLOCK -- registered 04:03 (before the binary exists), queued at the end (behind TR1c); H2-p LANDED (03:49) b60950b0de, falsifier RED; E2-q's first census on b1: SHELF MOVED 0; b2's TR1 night: THE PIT FIRED and the stall never released it

- H2-p chain b3hfjfahc: check, pin the_bed_earns_its_reputation, commit b60950b0de, both halves built, marker server=1, staged 03:49:42; the shipper put it in lab-bin at 03:49:54 (THE PAIR TO PLAY). Falsifier bphpe4u2m planted the bed always offered: "test result: FAILED. 0 passed; 1 failed" -> RED (good), tree restored clean at 03:54. E2-r's chain b38dveie2 took h2p-staged and committed 5e2887b40c (~03:58; building).
- E2-q on b1 (restarted 03:44 onto d894e982; day-0 boot census): SHELF MOVED 0 -- the registered NOTE branch: TR1 reads the mirror standing beside every shelf, so the placer's consumer is inert until the set refuses what the router refuses (TR1b), and then TR1c asks the router itself. SITE REACH CENSUS seeds 2,318, reached 150,000 (cap), sites 275, cut 4 = Designated 2 (work cells (7782,6307,180) and (7783,6307,181), Air above, jobs 55/56 unclaimed) + the same two store corners (Earth/Rock above); Bed 116/0; STAIR CENSUS 56/56.
- b2's TR1 night (day 1, pair 68e684fd, archived 03:49 as /tmp/arm-b2/prev-server-034915.log): in_bed 27/33/32/32/32/32/32 at hours 23-5 of roster 50 (lying 32, standing 0 at hour 0), to_bed 14/9/10/10/10/10/9 all night, watch 8, idle 0-1: a 32-of-50 night with ten walkers to bed for six hours (below the 37-42 of the W6-G nights; the loop class again). STRUCK OUT 3 (colonist 123 x1, 57 x2), shun 9, BEDS OUT 0 (the pair predates H2-p). b2 restarted 03:49 onto E2-q d894e982.
- THE PIT FIRED (its first firing in the four arm-nights since W18-i2 armed): THE BOB NAMES ITS COLUMN x7 for uid 134 at cell (7700,6303), z_from 183 -> z_to 181, site "bridge-probe", column from z 180 = [Rock, Rock, Rock, Empty, Empty, Empty], job 1242 RestAt bed (7769,6401,186) (70 blocks NE), same_job true at every power of two from 16 (03:33) to 1,024 (ticks_since_stall 7,608), clock_found 0.033 s at EVERY stall, reset_found 1e6. THE BOB IS A STALL fired at bobs 16, 17, 18, ... (the stall count climbing with every bob): the stall set the clock to the timeout and found it back at 0.033 s by the next bob. Colonist 20 stood "Traveling" at dist 165-174 within (7691-7700, 6303-6309, z 186) all night beside the same cell.
- READ IN THE CODE (the instrument named the site): the bridge-probe push (bastion_jobs ~38713) ends `if displaced { active.stuck_time = 0.0; }` -- a landed step longer than 0.3 of a walk step zeroes the clock, and a two-block bob is 2.0. W18-e2 pinned the RESET POINT (reset_dist = f32::MIN) against the lift's progress reset; this reset consults no reset point. And the stall sets `stuck_time = STUCK_TIMEOUT` while the conviction ladder (~39294) reads `stuck_time > STUCK_TIMEOUT` (strict): even unreset, the equal clock releases nothing until an accrual that runs only when no step lands. Two dead arms.
- W18-j (fix-w18j.py/.msg; chain bn60gzcyo on tr1c-staged; falsifier bzf8jf6kq plants the hold ignored; reader wait-stall-w18j b2hz3pmy4 keys on THE BOB IS A STALL): STALL_HOLD_TICKS 900; stall_clock_past(timeout) = timeout + 1; bob_reset_allowed(hold_until, now, displaced) = displaced && hold expired-or-none; board.stall_hold uid -> tick + 900 set at the stall; the probe's reset asks it and, refused, prints THE STALL HOLDS ITS CLOCK (uid, stuck_time, hold_until, tick, held); THE BOB NAMES ITS COLUMN gains landing + landing_column. Pin the_stall_holds_its_clock. Dry tree (HEAD 5e2887b40c + w14i7b + tr1b + tr1c + w18j) clean.
- PREDICTION (registered before the binary exists; the first arm-night a stall fires): a hold line within a tick of the stall (held >= 1); bobs on that job under 64 (a column line at 32 at most with the same job); the ladder's release for the uid within 30 s; no body at bobs 128 on either arm. FALSIFIED if a column line reads bobs >= 128 with same_job true and held > 0 (another reset site: its BODY BOBS site names it), or held stays 0 while bobs pass 64 (the probe is not the resetter). Falsified AS A FIX if the released body is back in the same pit within the hour (the route through the pit is the cut: the landing column names its shape; the geometry row follows).
- Rejected: gating all 31 reset sites blind; a longer STUCK_TIMEOUT; a lift out of the pit; clearing the hold on the release. NOT evidenced: why the probe drops the body two cells there (the landing column is the instrument); the ten walkers to bed all night on b2 (H2-p's first night reads them).
