...
boot)
  ...
  case "$ARM" in
    ...
    mega)
      # Ben 2026-08-24: "a big colony nearly city sized". Flat lab with the
      # town-size roll pinned to the generator's max (BASTION_TOWN_SIZE=1.0
      # — ~200 plot attempts, houses weighted 64/134) and population raised
      # to fill the housing. Same map splice as flat.
      sed -i "s/map_file: None,/map_file: Some(Generate((x_lg: 8, y_lg: 8, scale: 2.0, map_kind: Square, erosion_quality: 0.25))),/" "$S"
      grep -q "map_file: Some" "$S" || { echo "BOOT FAILED: mega-arm map_file splice did not take in $S" >&2; exit 4; }
      # VD 7 not 3 (measured: at 3 only the 30 core houses ever loaded/registered).
      # POP LADDER (Ben 2026-08-25): city footprint stays, population starts
      # at 24 and climbs rung by rung after each clean flyover. MUST match
      # PLAY.ps1 megatown or the observer measures a different colony.
      ENVV="BASTION_FLAT_WORLD=1 BASTION_TOWN_SIZE=1.0 BASTION_ADOPT_TOWN=1 BASTION_AUTOFOUND_REAL_TERRAIN=1 BASTION_COLONY_PRESENCE_VD=7 BASTION_AUTOFOUND_COLONY=24 BASTION_SEED_FOOD=256 BASTION_SEED_MATERIALS=256" ;;
