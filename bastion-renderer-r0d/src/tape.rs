...
//!
//! - §14.1 closed domain-rank registry (16 frozen ranks); unknown ranks rejected.
//! - §14.2 total order `(tick, frame_or_zero, domain_rank, authority_rank,
//!   owner_digest, leaf_kind_rank, local_ordinal)`; duplicate total keys rejected.
//! - §14.3 RFC 9162 tree: `leaf = SHA256(0x00 || record)`,
//!   `node = SHA256(0x01 || left || right)`; chunks of ≤256 records.
...
/// hashes the empty string; a single leaf uses the leaf hash; otherwise the tree
/// splits at the largest power of two strictly below the count.
#[must_use]
...
impl FinalizedTapeV1 {
    /// Finalize a set of locally-gathered records (§14.2/§14.3): validate ranks,
    /// sort by the total key, reject duplicate keys, then hash into chunk roots
    /// (≤256 records each) and the final RFC 9162 root over all leaves.
    pub fn finalize(mut records: Vec<TapeRecordV1>) -> Result<Self, TapeError> {
        for r in &records {
            if DomainRank::from_rank(r.key.domain_rank).is_none() {
                return Err(TapeError::UnknownDomainRank { rank: r.key.domain_rank });
            }
        }
    ...
    #[must_use]
    pub fn final_root(&self) -> [u8; 32] {
        self.final_root
    }

    #[must_use]
    pub fn chunk_roots(&self) -> &[[u8; 32]] {
        &self.chunk_roots
    }
    ...
    for ci in 0..nchunks {
        ...
        for i in 0..CHUNK_MAX {
            ...
            match (e, a) {
                ...
                (None, None) => break,
                _ => {
                    let key = e.map(|r| r.key).or_else(|| a.map(|r| r.key)).expect("one side present");
...
mod tests {
    ...
    fn finalize_sorts_and_is_producer_order_independent() {
        let a = FinalizedTapeV1::finalize(vec![rec(2, 3, 0, 9), rec(1, 3, 0, 8), rec(1, 2, 0, 7)]).unwrap();
        let b = FinalizedTapeV1::finalize(vec![rec(1, 2, 0, 7), rec(2, 3, 0, 9), rec(1, 3, 0, 8)]).unwrap();
    ...
    fn first_divergence_localizes_the_differing_record() {
        let a = FinalizedTapeV1::finalize(vec![rec(1, 2, 0, 1), rec(2, 3, 0, 2), rec(3, 3, 0, 3)]).unwrap();
        // Second record payload differs (9 vs 2).
        let b = FinalizedTapeV1::finalize(vec![rec(1, 2, 0, 1), rec(2, 3, 0, 9), rec(3, 3, 0, 3)]).unwrap();
