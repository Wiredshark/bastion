...
impl RendererPassGraphManifestV1 {
    ...
    pub fn build(nodes: Vec<PassNodeV1>, edges: Vec<HazardEdgeV1>) -> Result<Self, PassGraphError> {
        let mut map: BTreeMap<[u8; 32], PassNodeV1> = BTreeMap::new();
        for n in nodes {
            if map.insert(n.pass_digest, n.clone()).is_some() {
                return Err(PassGraphError::DuplicatePassDigest {
                    digest: n.pass_digest,
                });
            }
        }
        for e in &edges {
        ...
        let mut succ: BTreeMap<[u8; 32], Vec<[u8; 32]>> = BTreeMap::new();
        {
            ...
            let mut sorted_edges: Vec<(&HazardEdgeV1, ())> =
        ...
        if order.len() != map.len() {
            return Err(PassGraphError::CycleDetected {
                remaining: map.len() - order.len(),
            });
        }
        Ok(Self {
            nodes: map,
    ...
    #[must_use]
    pub fn canonical_order(&self) -> &[[u8; 32]] { &self.canonical_order }
    ...
    pub fn manifest_digest(&self) -> [u8; 32] {
        ...
        let mut es: Vec<([u8; 32], [u8; 32])> =
            self.edges.iter().map(|e| (e.before, e.after)).collect();
        es.sort();
        es.dedup();
    ...
    pub fn check_execution(&self, observed: &[[u8; 32]]) -> Result<(), ExecutionMismatch> {
        ...
        for (i, (exp, obs)) in self.canonical_order.iter().zip(observed.iter()).enumerate() {
            if exp != obs {
                return Err(ExecutionMismatch::OutOfOrder {
                    position: i,
                    expected: *exp,
                    observed: *obs,
                });
        ...
        match observed.len().cmp(&self.canonical_order.len()) {
            Less => Err(ExecutionMismatch::MissingPasses {
                count: self.canonical_order.len() - observed.len(),
            }),
            Greater => Err(ExecutionMismatch::ExtraPasses {
                count: observed.len() - self.canonical_order.len(),
            }),
            Equal => Ok(()),
        }
    }
...
mod tests {
    ...
    fn cycle_and_undeclared_are_typed() {
        ...
        assert_eq!(
            RendererPassGraphManifestV1::build(nodes.clone(), vec![edge(1, 2), edge(2, 1)])
                .unwrap_err(),
