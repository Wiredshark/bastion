...
impl Page {
    fn new(size: u32) -> Self {
        Self { free: vec![FreeRect { y: 0, x: 0, w: size, h: size }] }
    }
    ...
    fn place(&mut self, idx: usize, w: u32, h: u32) -> (u32, u32) {
        ...
        for f in &self.free {
            ...
            if placed.x + placed.w < f.x + f.w {
                next.push(FreeRect { x: placed.x + placed.w, y: f.y, w: f.x + f.w - (placed.x + placed.w), h: f.h });
            }
            if placed.y > f.y {
                next.push(FreeRect { x: f.x, y: f.y, w: f.w, h: placed.y - f.y });
            }
            if placed.y + placed.h < f.y + f.h {
                next.push(FreeRect { x: f.x, y: placed.y + placed.h, w: f.w, h: f.y + f.h - (placed.y + placed.h) });
            }
        }
...
pub fn pack_atlas(
    ...
    for r in inputs {
        ...
        if r.width > page_size || r.height > page_size {
            return Err(AtlasError::OversizedRect { width: r.width, height: r.height, page: page_size });
        }
    }
    ...
    for r in &sorted {
        ...
        let (page, x, y) = match placed {
            ...
            None => {
                ...
                let idx = page.find_bssf(r.width, r.height).expect("fits empty page (validated)");
                let (x, y) = page.place(idx, r.width, r.height);
                pages.push(page);
...
mod tests {
    ...
    fn rect(d: u8, w: u32, h: u32) -> AtlasInputV1 {
        AtlasInputV1 { content_digest: [d; 32], width: w, height: h }
    }
    ...
    fn encounter_order_cannot_change_placement() {
        let rects = vec![rect(1, 30, 40), rect(2, 50, 20), rect(3, 10, 10), rect(4, 25, 25)];
        let a = pack_atlas(&rects, 128, 4).unwrap();
        let mut rev = rects.clone();
        rev.reverse();
        let b = pack_atlas(&rev, 128, 4).unwrap();
        assert_eq!(atlas_digest(&a), atlas_digest(&b), "BTL-343: encounter order leaked");
    }

    #[test]
    fn placements_never_overlap_and_stay_in_bounds() {
        let rects: Vec<AtlasInputV1> =
            (0..40u8).map(|i| rect(i, 8 + u32::from(i % 13) * 3, 8 + u32::from(i % 7) * 5)).collect();
    ...
    fn oversized_and_empty_are_typed() {
        assert_eq!(
            ...
            AtlasError::OversizedRect { width: 65, height: 10, page: 64 }
        );
        assert_eq!(pack_atlas(&[rect(1, 0, 10)], 64, 1).unwrap_err(), AtlasError::EmptyRect);
    }

    #[test]
    fn frozen_layout_vector() {
        let placed = pack_atlas(
            &[rect(1, 30, 40), rect(2, 50, 20), rect(3, 10, 10)],
            128,
            2,
        )
        .unwrap();
