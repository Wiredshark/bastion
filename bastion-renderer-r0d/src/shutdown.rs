...
impl ShutdownStage {
    ...
    fn next(self) -> Option<ShutdownStage> { Self::ORDER.get(self.ordinal() + 1).copied() }
}
...
impl ShutdownSequenceV1 {
    ...
    #[must_use]
    pub fn stage(self) -> ShutdownStage { self.stage }

    fn is_terminal(self) -> bool { self.fault.is_some() || self.interrupted }

    /// Record the client close ack (produced at stage 8).
    pub fn record_client_ack(&mut self) { self.client_ack = true; }

    /// Record the server close ack (produced at stage 9).
    pub fn record_server_ack(&mut self) { self.server_ack = true; }
    ...
    pub fn advance(&mut self, to: ShutdownStage) -> Result<(), ShutdownError> {
        ...
        if to > ShutdownStage::ClientFlushAndAck && !self.client_ack {
            return Err(ShutdownError::MissingAck {
                stage: ShutdownStage::ClientFlushAndAck,
            });
        }
        if to > ShutdownStage::ServerStopAndAck && !self.server_ack {
            return Err(ShutdownError::MissingAck {
                stage: ShutdownStage::ServerStopAndAck,
            });
        }
        self.stage = to;
        Ok(())
    ...
    /// Record a typed fault (§19.2/§19.4). Once faulted the run can never
    /// succeed.
...
mod tests {
    ...
    fn out_of_order_advance_rejected() {
        ...
        assert_eq!(
            ...
            Err(ShutdownError::OutOfOrder {
                from: ShutdownStage::FreezeInputs,
                to: ShutdownStage::JoinWorkers
            })
        );
    }
    ...
    fn device_loss_can_never_succeed() {
        ...
        assert_eq!(
            s.verdict(),
            ShutdownVerdict::InvalidOrFailed(FaultTerminal::DeviceLost)
        );
    ...
    fn interrupt_is_never_success() {
        ...
        assert_eq!(
            s.advance(ShutdownStage::FreezeRequests),
            Err(ShutdownError::AlreadyTerminal)
        );
    }
}
