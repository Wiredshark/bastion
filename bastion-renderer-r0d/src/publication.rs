...
//! flush/sync/rename durability protocol (§15.3 steps / §15.4 platform tiers)
//! is the filesystem integration surface.
...
pub fn validate_logical_path(path: &str) -> Result<(), LogicalPathError> {
    ...
    for seg in path.split('/') {
        ...
        if WINDOWS_RESERVED.contains(&stem) {
            return Err(LogicalPathError::WindowsReservedName {
                name: stem.to_string(),
            });
        }
    }
...
/// A sorted publication index (§15.2). Entries sort by logical-path bytes,
/// never directory enumeration order; the digest binds the sorted descriptor
/// set.
#[derive(Clone, Debug)]
...
impl PublicationIndexV1 {
    ...
    pub fn build(mut descriptors: Vec<ArtifactDescriptorV1>) -> Result<Self, PublicationError> {
        for d in &descriptors {
            validate_logical_path(&d.logical_path).map_err(|reason| {
                PublicationError::InvalidPath {
                    path: d.logical_path.clone(),
                    reason,
                }
            })?;
        }
        ...
        for w in descriptors.windows(2) {
            if w[0].logical_path == w[1].logical_path {
                return Err(PublicationError::DuplicatePath {
                    path: w[0].logical_path.clone(),
                });
            }
        }
        Ok(Self { descriptors })
...
/// digests. Published last; a run is admissible only when a valid commit
/// matches.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
mod tests {
    ...
    fn rejects_each_bad_path() {
        ...
        assert_eq!(
            ...
            Err(WindowsReservedName {
                name: "nul".to_string()
            })
        );
        assert!(matches!(
            validate_logical_path(&"x".repeat(241)),
            Err(TooLong { .. })
        ));
        assert!(matches!(
    ...
    fn duplicate_logical_path_rejected() {
        assert_eq!(
            ...
            PublicationError::DuplicatePath {
                path: "r0d/a".to_string()
            }
        );
    }
