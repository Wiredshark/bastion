...
mod tests {
    ...
    fn mip_chain_terminates_at_1x1_and_data_planes_refuse_mips() {
        ...
        assert_eq!(
            build_mip_chain(&base, PlanePolicy::DataExact, false)
                .unwrap()
                .len(),
            1
        );
    ...
    fn payload_is_bit_reproducible_and_content_sensitive() {
        ...
        assert_ne!(
            payload_digest(&write_ktx2(&l1)),
            payload_digest(&write_ktx2(&l3))
        );
    }
}
