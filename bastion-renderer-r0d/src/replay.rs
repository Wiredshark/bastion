...
/// The exact-version replay identity (§16.1). V1 requires byte-exact equality
/// of every field; there is no compatibility range and no silent defaulting.
...
/// `ReplayVersionMismatch` (§16.1): the first field (frozen order) that
/// differs.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
impl ReplayVersionIdentityV1 {
    ...
    /// difference is a terminal `ReplayVersionMismatch` — cross-version replay
    /// is explicitly unsupported in V1.
    pub fn verify_exact(
        &self,
        candidate: &ReplayVersionIdentityV1,
    ) -> Result<(), ReplayVersionMismatch> {
        ...
        let checks = [
            ...
            (
                ...
                candidate.asset_package_root,
            ),
            (
                FeatureCapabilityTier,
                self.feature_capability_tier,
                candidate.feature_capability_tier,
            ),
            (
                ContractOracleSchema,
                self.contract_oracle_schema,
                candidate.contract_oracle_schema,
            ),
            (
                BootstrapManifest,
                self.bootstrap_manifest_digest,
                candidate.bootstrap_manifest_digest,
            ),
        ];
...
/// Checkpoint cadence (§16.3): a hash/index checkpoint every 256 simulation
/// ticks.
...
mod tests {
    ...
    fn any_field_drift_is_typed_mismatch() {
        ...
        assert_eq!(
            ...
            Err(ReplayVersionMismatch {
                field: ReplayVersionField::ExecutableDigest
            })
        );
    }
    ...
    fn checkpoint_digest_is_content_sensitive() {
        let a = CheckpointV1 {
            input_tape_offset: 10,
            semantic_tape_offset: 20,
            simulation_tick: 256,
            state_root: [7; 32],
            record_chunk_root: [8; 32],
        };
        let mut b = a;
        b.state_root = [9; 32];
