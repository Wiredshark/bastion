...
impl AgreementStateV1 {
    ...
    #[must_use]
    pub fn next(self) -> Option<AgreementStateV1> { Self::ORDER.get(self.ordinal() + 1).copied() }
    ...
    #[must_use]
    pub fn can_advance_to(self, to: AgreementStateV1) -> bool { self.next() == Some(to) }

    /// Nothing downstream (script phase, camera, readiness, capture) may
    /// proceed until both sides reach the terminal committed state (§10.2).
    #[must_use]
    pub fn is_committed(self) -> bool { self == AgreementStateV1::AgreementCommitted }
}
...
/// `SHA256("bastion/r0d/agreement/v1" || frame(Offer) || frame(Ack) ||
/// frame(Commit))`.
#[must_use]
...
mod tests {
    ...
    fn fragment_round_trip_is_order_independent() {
        ...
        assert!(
            frags.len() >= 3,
            "200KB splits into multiple 64KiB fragments"
        );
