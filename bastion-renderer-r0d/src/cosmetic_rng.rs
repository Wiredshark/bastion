...
use crate::bootstrap::{BootstrapErrorV1, hkdf_expand};
...
/// bumped by the Weyl constants between rounds. Byte/word order is
/// little-endian by the two-u32 key / four-u32 counter convention.
#[must_use]
...
/// Derive one cosmetic Philox sample by semantic tuple (§18.2).
/// `root_cosmetic_prk` is the HKDF PRK for the cosmetic root; the
/// effect/emitter/instance identity selects the owner material, and the
/// tick/ordinal/purpose select the counter. Direct random access — no shared
/// cursor, no order dependency.
#[must_use]
...
pub fn cosmetic_sample(
    ...
    purpose_lane: u32,
    sample_index: u32,
) -> Result<[u32; 4], BootstrapErrorV1> {
    // owner_material = HKDF-Expand(prk,
    // frame(label)||effect_kind||emitter||instance, 24)
    let mut info = Vec::with_capacity(2 + COSMETIC_LABEL.len() + 2 + 64);
    let label_length =
        u16::try_from(COSMETIC_LABEL.len()).map_err(|_| BootstrapErrorV1::LengthOverflow)?;
    info.extend_from_slice(&label_length.to_le_bytes());
    ...
    let low = u32::try_from(spawn_tick & u64::from(u32::MAX))
        .map_err(|_| BootstrapErrorV1::LengthOverflow)?;
    let high = u32::try_from(spawn_tick >> 32).map_err(|_| BootstrapErrorV1::LengthOverflow)?;
    let sample_counter = [
        base_counter[0] ^ low,
        base_counter[1] ^ high,
    ...
    Ok(philox4x32_10(sample_counter, philox_key))
}

/// Uniform `[0, 1)` conversion (§18.2): `(sample_u32 >> 8) * 2^-24`.
/// Presentation only unless the exact bits are certified.
#[must_use]
pub const fn u01_q24(sample_u32: u32) -> u32 { sample_u32 >> 8 }
...
mod tests {
    ...
    fn philox_frozen_all_zero() {
        assert_eq!(philox4x32_10([0, 0, 0, 0], [0, 0]), [
    ...
    fn philox_kat_all_ones() {
        assert_eq!(philox4x32_10([0xffff_ffff; 4], [0xffff_ffff; 2]), [
            0x408f_276d,
            0x41c8_3b0e,
            0xa20b_c7c6,
            0x6d54_51fd
    ...
    fn philox_kat_pi_vector() {
        assert_eq!(
            philox4x32_10([0x243f_6a88, 0x85a3_08d3, 0x1319_8a2e, 0x0370_7344], [
                0xa409_3822,
                0x299f_31d0
            ]),
    ...
    #[test]
    fn u01_q24_is_in_closed_fixed_point_range() {
        assert_eq!(u01_q24(0), 0);
        assert_eq!(u01_q24(u32::MAX), (1_u32 << 24) - 1);
    }
    ...
    fn authority_isolation_root_and_labels_are_separate() {
        ...
        let bootstrap_seed = {
            let reg = crate::bootstrap::SeedRegistryV1::new(shared_root, vec![
                crate::bootstrap::SeedDomainDeclarationV1::new(
                    "bastion/r0d/worldgen",
                    1,
                    0,
                    [0x22; 32],
                )
        ...
        let cosmetic = cosmetic_sample(&shared_root, 0, &[0; 32], &[0; 32], 0, 0, 0, 0).unwrap();
    ...
    fn neighboring_input_canaries_all_axes() {
        let base = cosmetic_sample(&[1; 32], 5, &[2; 32], &[3; 32], 100, 4, 1, 0).unwrap();
        // Every tuple axis independently changes the sample.
        assert_ne!(
            base,
            cosmetic_sample(&[9; 32], 5, &[2; 32], &[3; 32], 100, 4, 1, 0).unwrap(),
            "prk"
        );
        assert_ne!(
            base,
            cosmetic_sample(&[1; 32], 6, &[2; 32], &[3; 32], 100, 4, 1, 0).unwrap(),
            "kind"
        );
        assert_ne!(
            base,
            cosmetic_sample(&[1; 32], 5, &[9; 32], &[3; 32], 100, 4, 1, 0).unwrap(),
            "emitter"
        );
        assert_ne!(
            base,
            cosmetic_sample(&[1; 32], 5, &[2; 32], &[9; 32], 100, 4, 1, 0).unwrap(),
            "instance"
        );
        assert_ne!(
            base,
            cosmetic_sample(&[1; 32], 5, &[2; 32], &[3; 32], 101, 4, 1, 0).unwrap(),
            "tick"
        );
        assert_ne!(
            base,
            cosmetic_sample(&[1; 32], 5, &[2; 32], &[3; 32], 100, 5, 1, 0).unwrap(),
            "ordinal"
        );
        assert_ne!(
            base,
            cosmetic_sample(&[1; 32], 5, &[2; 32], &[3; 32], 100, 4, 2, 0).unwrap(),
            "purpose"
        );
        assert_ne!(
            base,
            cosmetic_sample(&[1; 32], 5, &[2; 32], &[3; 32], 100, 4, 1, 1).unwrap(),
            "index"
        );
    }
}
