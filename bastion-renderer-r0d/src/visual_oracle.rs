...
//! shadow-atlas tile assignment (§18.1). The GPU capture itself (R32Uint target,
//! warm RGBA readback) is the integration surface.
...
/// Assign opaque draw IDs by sorted structural draw order (§17.2). Input MUST be
/// pre-sorted by the canonical draw key; IDs are `1..=N` contiguous, and the
...
/// the changed-pixel count is within `max_changed_pixels`. There is no universal
/// threshold — the limits are per-test policy. Buffers are tightly-packed RGBA8.
#[must_use]
...
impl ShadowAtlasV1 {
    /// Place a caster by its sorted rank (§18.1). Deterministic function of rank
    /// alone; overflow past capacity is a typed error.
    pub fn place(&self, caster_rank: u32) -> Result<(u32, u32), ShadowAtlasOverflow> {
        ...
        if caster_rank >= capacity {
            return Err(ShadowAtlasOverflow { caster_rank, capacity });
        }
...
mod tests {
    ...
    fn entry(d: u8, pass: u16) -> OpaqueIdEntryV1 {
        OpaqueIdEntryV1 { entity_digest: [d; 32], pass_tag: pass, submesh_index: 0, material_tag: 0 }
    }
    ...
    fn environment_digest_is_member_sensitive() {
        let mut e = EnvironmentTupleV1 {
            os_build: [1; 32], backend: [2; 32], adapter: [3; 32], driver: [4; 32],
            shader_compiler: [5; 32], package_digests: [6; 32], features_limits: [7; 32],
    ...
    fn shadow_atlas_places_by_rank_and_overflows_typed() {
        let atlas = ShadowAtlasV1 { grid: 4, max_pages: 2 }; // 16 tiles/page, 32 capacity
        ...
        assert_eq!(atlas.place(32), Err(ShadowAtlasOverflow { caster_rank: 32, capacity: 32 }));
    }
}
