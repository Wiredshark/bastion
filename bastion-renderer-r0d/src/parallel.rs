...
pub fn split_tree(len: usize, grain: usize) -> Vec<LeafRange> {
    fn rec(start: usize, end: usize, grain: usize, out: &mut Vec<LeafRange>) {
        if end - start <= grain {
            out.push(LeafRange { ordinal: out.len() as u32, start, end });
        } else {
...
pub fn checked_sum_i64(items: &[i64], grain: usize) -> Result<i64, ReductionOverflow> {
    ...
    .expect("nonempty");
    if overflow { Err(ReductionOverflow) } else { Ok(total) }
}
...
pub fn stable_compact<T: Clone>(items: &[T], flags: &[bool]) -> Vec<T> {
    ...
    let count = scan.last().map_or(0, |&s| s + usize::from(*flags.last().unwrap_or(&false)));
    ...
    out.into_iter().map(|x| x.expect("scan slot filled")).collect()
}
...
mod tests {
    ...
    fn negative_canary_dynamic_completion_order_reduction_diverges() {
        ...
        for salt in 0..20u64 {
            let dynamic: f64 = permuted(leaves.len(), salt).into_iter().map(partial).fold(0.0, |a, b| a + b);
